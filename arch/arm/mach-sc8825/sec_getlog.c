			      ...
			      u32 frames)
{
	if (p_fb) {
		pr_info("%s: 0x%p %d %d %d %d\n", __func__, p_fb, xres, yres,
			bpp, frames);
		frame_buf_mark.p_fb = (p_fb - 0x84500000);
				  ...
				  void *p_radio, void *p_events, void *p_system)
{
	pr_info("%s: 0x%p 0x%p 0x%p 0x%p\n", __func__, p_main, p_radio,
		p_events, p_system);
		plat_log_mark.p_main = p_main-0x04500000;  // 0x2500000 means CONFIG_TEXT_OFFSET
		plat_log_mark.p_radio = p_radio-0x04500000;
		plat_log_mark.p_events = p_events-0x04500000;
		plat_log_mark.p_system = p_system-0x04500000;
}
...
{
	pr_info("%s: 0x%p\n", __func__, klog_buf);
	kernel_log_mark.klog_buf = klog_buf-0x04500000; // for SPRD
}
