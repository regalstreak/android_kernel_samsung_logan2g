...
#define GEN0_MASK (GEN0_SIM0_EN | /*GEN0_ADI_EN |*/ GEN0_GPIO_EN |		\
		...
		/*GEN0_SYST_EN |*/ GEN0_CCIR_MCLK_EN)
...
/* FPGA DEBUG ONLY */
#endif
//add for debug cp crash case TDFAE46065 start
#define SAVE_TIME_NUM 20
static u32 emc_repower_begin[SAVE_TIME_NUM];
static u32 emc_repower_end[SAVE_TIME_NUM];
static u32 emc_repower_saved_pointer = 0;
static void __init_repower_time(void)
{
	memset(emc_repower_begin, 0, sizeof(emc_repower_begin));
	memset(emc_repower_end, 0, sizeof(emc_repower_end));
}
static void __saved_repowered_time(u32 begin, u32 end)
{
	emc_repower_begin[emc_repower_saved_pointer] = begin;
	emc_repower_end[emc_repower_saved_pointer] = end;
	emc_repower_saved_pointer++;
	if(emc_repower_saved_pointer >= SAVE_TIME_NUM) {
		emc_repower_saved_pointer = 0;
	}
}
//add for debug cp crash case TDFAE46065 end
...
int deep_sleep(void)
{
	...
	*/
	...
	udelay(5);
	//modify for debug cp crash case TDFAE46065 start
	//if (ret) cpu_init();
	if (ret){
		__saved_repowered_time(repower_param->time_before, repower_param->time_after);
		cpu_init();
	}
	//modify for debug cp crash case TDFAE46065 end
...
#endif
#endif
	dmc_freq = emc_freq_get();
	if((dmc_freq > 200)) {
		/*if dmc freq > 200M the chip can't go to deep sleep*/
		status |= DEVICE_APB;
	}
...
#ifdef CONFIG_NKERNEL
	/*force close cp*/
	/* delete force cp to sleep, because emc change program will do this, 
	 *delete this for avoid conflict
...
static void sc8825_power_off(void)
{
	if(usb_get_vbus_state()){
		sci_adi_raw_write(ANA_REG_GLB_RST_STATUS, 0);
...
void sc8825_pm_init(void)
{
	...
	fpga_dbg_init();
#endif
	__init_repower_time();	//add for debug cp crash case TDFAE46065
	init_gr();
#if CONFIG_NR_CPUS > 1
