...
ssize_t sec_tsp_intensity_log_on(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    tsp_info("tsp intensity logging on : Ready");
...
ssize_t sec_tkey_zvalue_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    u32 zvalue = ist30xx_key_sensitivity;
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    tsp_info("tkey zvalue val: %x, cmd: %d\n", val, cmd);
