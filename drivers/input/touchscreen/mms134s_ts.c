...
#ifdef CONFIG_MACH_NEVISTD
#define GPIO_HW_REV_0 45
#define GPIO_HW_REV_1 46
#define Main_Board_HW_REV_01 1
#endif
...
unsigned char TSP_PanelHWVersion, TSP_PhoneHWVersion;
#ifdef CONFIG_MACH_NEVISTD
unsigned char Main_Board_HW_Version;
#endif
...
static void mms_set_noise_mode(struct mms_ts_data *ts)
{
	...
	mms_i2c_write(ts->client, setLowLevelData, 2);
	msleep(10);
}
#endif
...
//	check_firmware_version_func();
	...
	return sprintf(buf, "ME%02x%04x\n", TSP_PhoneHWVersion, NEW_FIRMWARE_VERSION);
}
...
static ssize_t firmware_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (0 == mms_i2c_read(ts->client, MIP_TSP_REVISION, 6, buf1))
	{
		...
		printk(KERN_ERR "[TSP][MMS128s][%s]  phone_ver=%d, fw_ver=%d, hw_rev=%d\n",
			buf, phone_ver, fw_ver, hw_rev);
	} else {
		printk(KERN_ERR "[TSP][MMS128s][%s] Can't find HW Ver, FW ver!\n",
				__func__);
	}
...
static void get_chip_name(void *device_data)
{
	...
	set_default_result(ts);
#if defined(CONFIG_MACH_KYLETD) || defined(CONFIG_MACH_VASTOI) || defined(CONFIG_MACH_NEVISTD)
	snprintf(buff, sizeof(buff), "%s", "MMS128S");
#else
...
#ifdef CONFIG_MACH_NEVISTD
unsigned char check_board_HW_version(void)
{
	unsigned char ret, hw_rev_0=-1, hw_rev_1=-1;
	unsigned char mainboard_hw_rev;

	ret = gpio_request(GPIO_HW_REV_0, "hw_rev_0");

	if (ret) 
		printk("detect_device fail to request gpio %d\n", ret);
	else
	{
		gpio_direction_input(GPIO_HW_REV_0);
	
		hw_rev_0 = gpio_get_value(GPIO_HW_REV_0);
	}

	ret = gpio_request(GPIO_HW_REV_1, "hw_rev_1");

	if (ret) 
		printk("detect_device fail to request gpio %d\n", ret);
	else
	{
		gpio_direction_input(GPIO_HW_REV_1);
	
		hw_rev_1 = gpio_get_value(GPIO_HW_REV_1);
	}

	mainboard_hw_rev = (hw_rev_0 << 1) | hw_rev_1 ;

	gpio_free(GPIO_HW_REV_0);
	gpio_free(GPIO_HW_REV_1);
...
#ifdef CONFIG_MACH_NEVISTD
	Main_Board_HW_Version = check_board_HW_version();
#endif
...
//		if ((IC_type == 0x0F && crc_b2 != G1F_V20_CRC) ||(IC_type == 0x0D &&  crc_b2 != G1M_V34_CRC))  {
	...
	if (IC_type == 0x0F)
		...
		#else defined(CONFIG_MACH_NEVISTD)
		if(Main_Board_HW_REV_01 <= Main_Board_HW_Version)
		ts->config_fw_version = "S6818_ME_0225_PFF";
		else
		ts->config_fw_version = "S6818_ME_0131_G1F";
		#endif
	}
...
#endif
	{
		printk("melfas tsp chipset applied\n");
		FW_VERSION = 0x19;
