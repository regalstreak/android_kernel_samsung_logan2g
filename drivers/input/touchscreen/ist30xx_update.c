...
u8 *ist30xx_fw_b;       // temp, ist30xxb
u8 *ist30xx_param_b;    // temp, ist30xxb
...
int ist30xx_check_auto_update(struct ist30xx_data *data)
{
	...
	// for Multiple TSP
	if (ist30xx_tsp_type == 0) {    // tsp_type: 0 --> IST3000, 1 --> IST3000B
		ts_fw = ist30xx_fw_b;
		ts_param = ist30xx_param_b;
		ts_fw_size = sizeof(ist30xx_fw_b);
		ts_param_size = sizeof(ist30xx_param_b);
	}
	tsp_info("tsp type: %d, size: %d\n", ist30xx_tsp_type, ts_fw_size);
