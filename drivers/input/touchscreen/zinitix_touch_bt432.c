...
#include "zinitix_touch_zxt_firmware(ZI122207).h"
#include "zinitix_touch_zxt_firmware(ZI212207).h"
#endif
...
#if FACTORY_MODE_CHECKING
static u8 Factory_Mode = 0;
static u8 Factory_Count = 0;
#endif
...
/* Post Sensitivity Timer : Prevent ghost touch */
#define USE_POST_SENSITIVITY
#define	POST_SENSITIVITY_TIME	60*30	//sec
#define	POST_SENSITIVITY	130	//view area
#define POST_SENSITIVITY_BTN	280	//button


/* interrupt pin number*/
 #define GPIO_TOUCH_PIN_NUM	141
 #define TSP_HW_GPIO_NUM	142

#define TSP_TYPE_COUNT	2
const u8 *m_pFirmware [TSP_TYPE_COUNT] = {(u8*)m_firmware_data_12,(u8*)m_firmware_data_21,};
u8 m_FirmwareIdx = 0;
#define TSP_HW_ID_INDEX_0 0x12
#define TSP_HW_ID_INDEX_1 0x21
#define TSP_HW_ID_INDEX_2 0x11
...
static u16 touchkey_regulator_cnt;
...
static ssize_t factory_mode_enable(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t factory_mode_disable(struct device *dev, struct device_attribute *attr, char *buf);
...
static DEVICE_ATTR(tsp_firm_update, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, firmware_update, NULL);
static DEVICE_ATTR(tsp_firm_update_status, S_IRUGO | S_IWUSR | S_IWGRP | S_IXOTH, firmware_update_status, NULL);

static DEVICE_ATTR(tsp_factory_enable, S_IRUGO | S_IWUSR | S_IWGRP, factory_mode_enable, NULL);
static DEVICE_ATTR(tsp_factory_disable, S_IRUGO | S_IWUSR | S_IWGRP, factory_mode_disable, NULL);
...
#if FACTORY_MODE_CHECKING
static void checking_factory_mode(void)
{
	...
	filp = filp_open("/efs/FactoryApp/factorymode", O_RDONLY, 0);
	...
	dp = kmalloc(l+5, GFP_KERNEL);		/* add cushion : origianl code is 'dp = kmalloc(l, GFP_KERNEL);' */
	...
	ret_fs = vfs_read(filp, (char __user *)dp, l, &pos);
	...
	zinitix_printk(" factory mode is %s !!!\n",dp);


      if(dp[1] == 'F')
      {
        Factory_Mode = 1;
        zinitix_printk(" factory mode is 1 !!!\n");
      }

      if(dp[1] == 'N')
      {
        Factory_Mode = 0;
        zinitix_printk(" factory mode is 0 !!!\n");
      }
...
static bool ts_read_coord(struct zinitix_touch_dev *touch_dev)
{
	...
	if (touch_dev->touch_mode != TOUCH_POINT_MODE) {
		...
		if (ts_get_raw_data(touch_dev) == false)
			...
			touch_dev->touch_info.status = 0x0;
		}
...
#if FACTORY_MODE_CHECKING      /* 1*/  
		if(Factory_Count != 100)
			Factory_Count++;
        
		if(Factory_Count == 10){
			Factory_Count = 100;
			checking_factory_mode();
...
static bool ts_power_control(struct zinitix_touch_dev *touch_dev, u8 ctl)
{
	...
	if (tsp_regulator_3_3 == NULL){
		tsp_regulator_3_3 = regulator_get(NULL, "vddcammot");
...
static bool ts_mini_init_touch(struct zinitix_touch_dev *touch_dev)
{
	...
	if(touch_dev->cap_info.hw_id == TSP_HW_ID_INDEX_1) {
		if (ts_write_reg(touch_dev->client,0x00FD, 2400) != I2C_SUCCESS)
			goto fail_mini_init;
		if (ts_write_reg(touch_dev->client,0x0109, (u16)(-2400)) != I2C_SUCCESS)
			goto fail_mini_init;
		if (ts_write_reg(touch_dev->client,0x0105, 2400) != I2C_SUCCESS)
			goto fail_mini_init;
		if (ts_write_reg(touch_dev->client,0x011e, 0x9d17) != I2C_SUCCESS)
			goto fail_mini_init;
	}
...
static void ts_post_sensitivity_handler(unsigned long data)
{
	...
	if (ts_write_reg(touch_dev->client,
			ZINITIX_BUTTON_SENSITIVITY, POST_SENSITIVITY_BTN) != I2C_SUCCESS) {
		zinitix_printk("failed to write post btn sensitivity.\n");
	}
}

static void ts_post_sensitivity_timer_start(u16 sec, struct zinitix_touch_dev *touch_dev)
{
	if(touch_dev == NULL)	return;
	...
	//zinitix_printk("post timer start.\n");
}
...
static bool ts_hw_calibration(struct zinitix_touch_dev *touch_dev)
{
	...
	if( (touch_dev->cap_info.ModulId == 0x6565) || (touch_dev->cap_info.ModulId == 0x4c49)
		|| (touch_dev->cap_info.ModulId == 0x5359) ) {
		ts_write_reg(touch_dev->client,	ZINITIX_TSP_MODUL_ID,
			touch_dev->cap_info.ModulId);
	}
	if (ts_write_reg(touch_dev->client,
...
retry_init:
	...
	if (ts_read_data(touch_dev->client,
		ZINITIX_TSP_MODUL_ID,
		(u8 *)&touch_dev->cap_info.ModulId, 2) < 0)
		goto fail_init;
	zinitix_printk("touch TSP modul ID = %x\n",
		touch_dev->cap_info.ModulId);
...
fail_init:
	...
	if (++retry_cnt <= ZINITIX_INIT_RETRY_CNT) {
		...
		if( touch_dev->cap_info.hw_id==TSP_HW_ID_INDEX_0 ) {
			if (gpio_get_value(TSP_HW_GPIO_NUM)) {
				zinitix_printk("HW_ID mismatch %x, GPIO %d.\n", TSP_HW_ID_INDEX_0, 1);
				return false;
			}
		}
		else if( touch_dev->cap_info.hw_id==TSP_HW_ID_INDEX_1 ) {
			if (!gpio_get_value(TSP_HW_GPIO_NUM)) {
				zinitix_printk("HW_ID mismatch %x, GPIO %d.\n", TSP_HW_ID_INDEX_1, 0);
				return false;
			}
		}
		else if( touch_dev->cap_info.hw_id==TSP_HW_ID_INDEX_2 ) {
			zinitix_printk("HW_ID %x.\n", TSP_HW_ID_INDEX_2);
			return false;
		}
		else
		{
			if (gpio_get_value(TSP_HW_GPIO_NUM))
				touch_dev->cap_info.hw_id = TSP_HW_ID_INDEX_1;
			else
				touch_dev->cap_info.hw_id = TSP_HW_ID_INDEX_0;
			zinitix_printk("unknown HW_ID %x.\n", touch_dev->cap_info.hw_id);
		}
#else
		ts_check_hwid_in_fatal_state(touch_dev);
		if( touch_dev->cap_info.hw_id==TSP_HW_ID_INDEX_2 ) {
					zinitix_printk("HW_ID %x.\n", TSP_HW_ID_INDEX_2);
					return false;
#endif		
		ts_select_type_hw(touch_dev);
...
static void get_key_threshold(void *device_data)
{
	...
	if(misc_touch_dev->cap_info.hw_id == TSP_HW_ID_INDEX_1)
...
static void run_reference_read(void *device_data) //SDND
{
	...
	for(i=0; i<info->cap_info.x_node_num; i++)
	{
		...
		{
                        printk("%d ", info->dnd_data[j+i*info->cap_info.y_node_num]);

			if((info->dnd_data[j+i*info->cap_info.x_node_num] < min) && (info->dnd_data[j+i*info->cap_info.x_node_num]!=0))
			{
				min = info->dnd_data[j+i*info->cap_info.x_node_num];
			}

			if(info->dnd_data[j+i*info->cap_info.x_node_num] > max)
			{
				max = info->dnd_data[j+i*info->cap_info.x_node_num];
			}
...
static ssize_t firmware_update(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ret =  ts_upgrade_sequence((u8*)m_pFirmware[1]);
...
static ssize_t factory_mode_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (ts_write_reg(misc_touch_dev->client,0x01ca, 0) != I2C_SUCCESS)
	{
		zinitix_printk("%s fail !!!\n",__func__);
		return snprintf(buf, sizeof(buf), "FAIL");
	}

	if (ts_write_reg(misc_touch_dev->client,0x011e, 0x8517) != I2C_SUCCESS)
	{
		zinitix_printk("%s fail !!!\n",__func__);
		return snprintf(buf, sizeof(buf), "FAIL");
	}

	if (ts_write_reg(misc_touch_dev->client,0x003f, 0x2424) != I2C_SUCCESS)
	{
		zinitix_printk("%s fail !!!\n",__func__);
		return snprintf(buf, sizeof(buf), "FAIL");
	}

	return snprintf(buf, sizeof(buf), "OK");
}
...
static ssize_t factory_mode_disable(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(misc_touch_dev->cap_info.hw_id == TSP_HW_ID_INDEX_0) {
		if (ts_write_reg(misc_touch_dev->client,0x011e, 0x8717) != I2C_SUCCESS)
	{
		zinitix_printk("%s fail !!!\n",__func__);
		return snprintf(buf, sizeof(buf), "FAIL");
	}
		...
		if (ts_write_reg(misc_touch_dev->client,0x011e, 0x9d17) != I2C_SUCCESS)
		{
			zinitix_printk("%s fail !!!\n",__func__);
			return snprintf(buf, sizeof(buf), "FAIL");
		}
	...
	if (ts_write_reg(misc_touch_dev->client,0x003f, 0x1910) != I2C_SUCCESS)
	{
		zinitix_printk("%s fail !!!\n",__func__);
		return snprintf(buf, sizeof(buf), "FAIL");
	}
...
#if CHECK_HWID_PIN
	ret = gpio_request(TSP_HW_GPIO_NUM, "zinitix_hwid_gpio");
	if (ret) {
		zinitix_printk("unable to request gpio.\r\n");
		//goto err_power_sequence;
	}
	ret = gpio_direction_input(TSP_HW_GPIO_NUM);
#endif
...
//build error
	...
	if (device_create_file(sec_touchscreen_dev, &dev_attr_tsp_firm_update_status) < 0)
    	...
    	if (device_create_file(sec_touchscreen_dev, &dev_attr_tsp_factory_enable) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_tsp_factory_enable.attr.name);
	if (device_create_file(sec_touchscreen_dev, &dev_attr_tsp_factory_disable) < 0)
		pr_err("[TSP] Failed to create device file(%s)!\n", dev_attr_tsp_factory_disable.attr.name);
...
#ifdef CONFIG_ARCH_SC8825
	sprd_i2c_ctl_chg_clk(0, 400000);
#else
	sc8810_i2c_set_clk(0, 400000);
#endif
