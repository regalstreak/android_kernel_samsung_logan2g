...
#include <NEVIS_CHINA__PFF_R03_V08_bin.c>
...
extern unsigned char TSP_PanelHWVersion, TSP_PhoneHWVersion;
#ifdef CONFIG_MACH_NEVISTD
extern unsigned char Main_Board_HW_Version;
#endif
unsigned char IC_type;
static int cal_mode;
...
eMFSRet_t check_module_compatibility(const unsigned char *_pBinary_Data)
{
	...
	mms_fw_i2c_read(MODULE_COMPATIBILITY_ADDR, &read_buffer, 1);
	moduleComp = read_buffer;
	TSP_PanelHWVersion = moduleComp;
	printk(KERN_ERR "<MELFAS> Module Compatibility moduleComp=%x\n",
		moduleComp);

	setComp = _pBinary_Data[SET_COMPATIBILITY_ADDR];
	TSP_PhoneHWVersion = setComp;
	printk(KERN_ERR "<MELFAS> Module Compatibility setComp=%x, IC_type=%x\n", setComp,IC_type);
