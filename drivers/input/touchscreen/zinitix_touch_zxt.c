...
#include "zinitix_touch_zxt.h"
#if MULTI_PROTOCOL_TYPE_B
#include <linux/input/mt.h>
#endif
#include <mach/regulator.h>

#include "zinitix_touch_zxt_firmware_ZI112208.h"
...
/* interrupt pin number*/
 #define GPIO_TOUCH_PIN_NUM	60//GPIO_TSP_INT
...
static ssize_t is_during_call(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
...
static DEVICE_ATTR(during_call_for_tsp ,S_IRUGO | S_IWUGO , NULL, is_during_call);
...
static bool ts_set_touchmode(u16 value){
	...
	if(value == TOUCH_DND_MODE) {
		...
		if (ts_write_reg(misc_touch_dev->client, 0x12b, 2)!=I2C_SUCCESS)
			printk(KERN_INFO "[zinitix_touch] TEST Mode : Fail to set ZINITIX_AFE_R_SHIFE_VALUE %d.\r\n", 2);
	...
	if(misc_touch_dev->touch_mode == TOUCH_POINT_MODE){
		...
		if (ts_write_reg(misc_touch_dev->client, 0x12b, 3)!=I2C_SUCCESS)
			printk(KERN_INFO "[zinitix_touch] TEST Mode : Fail to set ZINITIX_AFE_R_SHIFE_VALUE %d.\r\n", 3);
...
static void run_reference_read(void *device_data)
{
	...
	for(i=0; i<info->cap_info.x_node_num; i++)
	{
		...
		{
			printk("[TSP] info->dnd_data : %d \n", info->dnd_data[j+i*info->cap_info.y_node_num]);
	...
	printk(" raw_min= %u, raw_max= %u\n", min, max);
	snprintf(buff, sizeof(buff),"%u,%u", min, max);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void run_normal_read(void *device_data)
{
	...
	unsigned int min, max;
	int i,j;
	min = info->normal_data[0];
	max = info->normal_data[0];

	for(i=0; i<info->cap_info.x_node_num; i++)
	{
		for(j=0; j<info->cap_info.y_node_num; j++)
		{
			printk("[TSP] info->normal_data : %d \n", info->normal_data[j+i*info->cap_info.y_node_num]);

			if(info->normal_data[j+i*info->cap_info.y_node_num] < min)
			{
				if(i < info->cap_info.x_node_num-1)
					min = info->normal_data[j+i*info->cap_info.y_node_num];
				else
				{
					if(j==2||j==7)
						min = info->normal_data[j+i*info->cap_info.y_node_num];
			...
			if(info->normal_data[j+i*info->cap_info.y_node_num] > max)
			{
				if(i < info->cap_info.x_node_num-1)
					max = info->normal_data[j+i*info->cap_info.y_node_num];
				else
				{
					if(j==2||j==7)
						max = info->normal_data[j+i*info->cap_info.y_node_num];
	...
	printk(" normal_data raw_min= %u, raw_max= %u\n", min, max);
	snprintf(buff, sizeof(buff),"%u,%u", min, max);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
...
static void run_delta_read(void *device_data)
{
	...
	min = info->delta_data[0];
	max = info->delta_data[0];

	for(i=0; i<info->cap_info.x_node_num; i++)
	{
		for(j=0; j<info->cap_info.y_node_num; j++)
		{
			printk("[TSP] info->delta_data : %d \n", info->delta_data[j+i*info->cap_info.y_node_num]);

			if(info->delta_data[j+i*info->cap_info.y_node_num] < min)
			{
				if(i < info->cap_info.x_node_num-1)
					min = info->delta_data[j+i*info->cap_info.y_node_num];
				else
				{
					if(j==2||j==7)
						min = info->delta_data[j+i*info->cap_info.y_node_num];
			...
			if(info->delta_data[j+i*info->cap_info.y_node_num] > max)
			{
				if(i < info->cap_info.x_node_num-1)
					max = info->delta_data[j+i*info->cap_info.y_node_num];
				else
				{
					if(j==2||j==7)
						max = info->delta_data[j+i*info->cap_info.y_node_num];
	...
	printk("delta_data raw_min= %u, raw_max= %u\n", min, max);
	snprintf(buff, sizeof(buff),"%u,%u", min, max);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
...
static void run_intensity_read(void *device_data)
{
	...
	printk(" run_intensity_read raw_min= %u, raw_max= %u\n", min, max);
	snprintf(buff, sizeof(buff),"%u,%u", min, max);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
...
static ssize_t is_during_call(struct device *dev, struct device_attribute
				  *attr, const char *buf, size_t count)
{
	int during_call = atoi(buf);
	
	printk("is_during_call during_call=%d\n",during_call);

	if(during_call == 1)
	{
		ts_write_reg(misc_touch_dev->client,0x0147,0x0001);

	}
	else
	{
		ts_write_reg(misc_touch_dev->client,0x0147,0x0000);
	}
...
//		IRQF_TRIGGER_FALLING, ZINITIX_DRIVER_NAME, touch_dev);
	...
	if (device_create_file(sec_touchscreen_dev,
			&dev_attr_during_call_for_tsp) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_during_call_for_tsp.attr.name);
#endif
...
static int rev_hw=0;
extern unsigned char check_board_HW_version(void);
#endif
static int __devinit zinitix_touch_init(void)
{
	printk("zinitix_touch_init\n");
#ifdef CONFIG_MACH_NEVISTD
	rev_hw = check_board_HW_version();
	if(rev_hw>=2)
#endif
	{
		printk("zinitix tsp chipset applied\n");
#if TOUCH_I2C_REGISTER_HERE	
