...
#include "zinitix_touch_bt4x4_firmware_R06.h"


#define	ZINITIX_DEBUG		0
#define	TOUCH_PRINT		0
...
#endif
#if SEC_TSP_FACTORY_TEST
	s16 delta_data[MAX_TEST_RAW_DATA];
	u16 scan_data[MAX_TEST_RAW_DATA];
	...
	u16 reg, u8 *values, u16 length)
{
	...
	pkt[2] = values[0];
	pkt[3] = values[1];
	ret = ts_retry_send(client , pkt , length+2);
	if (ret < 0)
		return ret;
	udelay(DELAY_FOR_POST_TRANSCATION);
	return length;
	...
	char *addr, u8 *values, u16 length)
{
	...
	if (addr != NULL) {
		/* select register*/
		ret = ts_retry_send(client , addr , 2);
		if (ret < 0)
			return ret;
		/* for setup tx transaction. */
		mdelay(1);
...
{
	s32 ret;
	ret = ts_retry_send(client , values , length);
	if (ret < 0)
		return ret;
	udelay(DELAY_FOR_POST_TRANSCATION);
	return length;
...
//		;
	...
	if(touch_regulator_3v0 == NULL) {
		touch_regulator_3v0 = regulator_get(NULL,"lcd_vcc");
...
#endif
	gpio_direction_output(TSP_EN, ctl);
	printk("[TSP]%s : %d\n",__func__, ctl);
...
#if 1
	printk("[TSP]decreased filter depth value ++\n");
	if (ts_write_reg(touch_dev->client,
		0x31, 8) != I2C_SUCCESS)
		goto fail_mini_init;
	if (ts_write_reg(touch_dev->client,
...
/* get chip firmware version */
	...
	if (ts_read_data(touch_dev->client,	ZINITIX_TSP_TYPE,(u8 *)&chip_hwid_info, 2) < 0)
		goto fail_mini_init;
	zinitix_debug_msg("touch hwID info = 0x%04X\r\n", chip_hwid_info);	
	...
	touch_dev->cap_info.chip_hwid_version = (u16) chip_hwid_info;
...
//	u16	newRegVersion;
	...
	if (ts_read_data(misc_touch_dev->client, ZINITIX_DATA_VERSION_REG, (u8*)&phoneVersion, 2)<0)
		printk("[TSP]read data fail! (%s)\n",__func__);
	...
	zinitix_printk("[TSP] phone Version = %d, sw Version = %d\n", phoneVersion, swVersion);
	...
	if((curVersion != 0x86) && newVersion < 4 ) return true;
	if(phoneVersion < swVersion) return true;
	else if(curVersion < newVersion) return true;	// temporal modification
	else if(phoneVersion > 0xFF) return true;
	else return false;
#if 0
...
#define	MAX_X_NODE_CNT		18
#define	MAX_Y_NODE_CNT		10
#define	ZINITIX_MENU_KEY        178
#define	ZINITIX_BACK_KEY        171
#define	CAL_MIN_NUM		1400
#define	CAL_MAX_NUM		2030
#define	INT_WAIT_TIME		50

#define	CHIP_INFO		"ZINITIX"
#define	CHIP_NAME		"BT404"
...
{
	struct zinitix_touch_dev *info = (struct zinitix_touch_dev *)device_data;
	u16 newVersion, newRegVersion, newHWID;
	u32 version;
	char buff[16] = {0};
	...
	newRegVersion = (u16) (m_firmware_data[0] | (m_firmware_data[1]<<8));
	newHWID = (u16) (m_firmware_data[0x6414]);

	version = (u32)((u32)(newHWID&0xff)<<16)|((newRegVersion&0xff)<<8)|(newVersion&0xff);
...
static void get_fw_ver_ic(void *device_data)
{
	...
	newVersion = info->cap_info.chip_firmware_version;
	newRegVersion = info->cap_info.chip_reg_data_version;
	newHWID = info->cap_info.chip_hwid_version;
//	if (ts_read_data(misc_touch_dev->client,	0x14,(u8 *)&newHWID, 2) < 0)
//		printk("[TSP]read data fail! (%s)\n",__func__);
	version = (u32)((u32)(newHWID&0xff)<<16)|((newVersion&0xff)<<8)|(newRegVersion&0xff);
...
static void get_chip_name(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s",CHIP_NAME);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void get_threshold(void *device_data)
{
	...
	ret = ts_read_data(info->client, 0x0020, (u8*)&threshold, 2);
...
static void get_key_threshold(void *device_data)
{
	...
	ret = ts_read_data(misc_touch_dev->client, 0x00B2, (u8*)&threshold, 2);
...
static int ts_read_raw_datas(void *device_data, u16 type)
{
	struct zinitix_touch_dev *info = (struct zinitix_touch_dev *)device_data;
	...
	u8 *cur;
	int num;
	u16 buf[MAX_TEST_RAW_DATA] = {0, };

/* raw data type checking */

	ret = ts_read_data(client, ZINITIX_TOUCH_MODE, (u8 *)&old_mode, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s: err: rd (touch mode)\n", __func__);
		goto out;
	}

	ret = ts_write_reg(client, ZINITIX_TOUCH_MODE,type);
	if (ret < 0) {
	...
	for (i = 0; i < 3; i++) {
		ts_write_cmd(client, ZINITIX_CLEAR_INT_STATUS_CMD);
		msleep(100);
	}

	while (true) {
		if (!gpio_get_value(info->int_gpio_num))
			break;
		msleep(1);
	}
	num = (type == TOUCH_ZINITIX_CAL_N_MODE) ?
				info->cap_info.total_cal_n : MAX_TEST_RAW_DATA;

	ret = ts_read_raw_data(info->client, ZINITIX_RAWDATA_REG,
	...
	ret = ts_write_reg(client, ZINITIX_TOUCH_MODE, old_mode);
	if (ret < 0) {
		dev_err(&client->dev, "%s: err: wt reg (touch mode)\n",
								__func__);
		goto out;
	...
	if (type == TOUCH_ZINITIX_CAL_N_MODE)
	{
		j = 0;
		cur = (u8 *)&buf[0];
		for (i = 0; i < info->cap_info.total_cal_n; i++)
		{
			if ((i * 2) % info->cap_info.max_y_node <	MAX_Y_NODE_CNT)
			{
				info->scan_data[j++] = (u16)cur[i * 2];
				info->scan_data[j++] =
							(u16)cur[i * 2 + 1];
				msleep(1);
				if (j >= MAX_TEST_RAW_DATA)
					break;
			}
		}
	}
	else
	{
		for (i = 0; i < num; i++)
		{
			switch (type)
			{
			case TOUCH_ZINITIX_PROCESSED_RAW_MODE:
				info->delta_data[i] = buf[i];
				break;
			case TOUCH_ZINITIX_CAL_N_DATA_MODE:
				info->ref_data[i] = buf[i];
				break;
			default:
				dev_err(&client->dev,
...
static void run_reference_read(void *device_data)
{
	...
	memset(info->ref_data, 0x00, sizeof(info->ref_data[0]) *ARRAY_SIZE(info->ref_data));

	ret = ts_read_raw_datas(info, TOUCH_ZINITIX_CAL_N_DATA_MODE);
	if (ret < 0)
		goto err;

	raw_min = raw_max = info->ref_data[0];

	for (i = 0; i < MAX_X_NODE_CNT; i++) {
		for (j = 0; j < MAX_Y_NODE_CNT; j++) {

			raw_data = info->ref_data[i * MAX_Y_NODE_CNT + j];
			if (i < MAX_X_NODE_CNT - 1) {
				if (raw_max < raw_data)
					raw_max = raw_data;
	...
	snprintf(buff, sizeof(buff), "%d,%d", raw_min, raw_max);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
...
err:
	...
	dev_err(&client->dev, "%s: fail to read reference data\n", __func__);
	return ;
}
static void run_delta_read(void *device_data)
{
	...
	memset(info->delta_data, 0x00, sizeof(info->delta_data[0]) *ARRAY_SIZE(info->delta_data));

	ret = ts_read_raw_datas(info, TOUCH_ZINITIX_PROCESSED_RAW_MODE);
	if (ret < 0)
		goto err;

	raw_min = raw_max = info->delta_data[0];

	for (i = 0; i < MAX_X_NODE_CNT; i++) {
		for (j = 0; j < MAX_Y_NODE_CNT; j++) {

			raw_data = info->delta_data[i * MAX_Y_NODE_CNT + j];
			if (i < MAX_X_NODE_CNT - 1) {
				if (raw_max < raw_data)
					raw_max = raw_data;
...
static void run_scantime_read(void *device_data)
{
	...
	memset(info->scan_data, 0x00, sizeof(info->scan_data[0]) *ARRAY_SIZE(info->scan_data));

	ret = ts_read_raw_datas(info, TOUCH_ZINITIX_CAL_N_MODE);
	if (ret < 0)
		goto err;

	raw_min = raw_max = info->scan_data[0];

	for (i = 0; i < MAX_X_NODE_CNT; i++) {
		for (j = 0; j < MAX_Y_NODE_CNT; j++) {

			raw_data = info->scan_data[i * MAX_Y_NODE_CNT + j];
			if (i < MAX_X_NODE_CNT - 1) {
				if (raw_max < raw_data)
					raw_max = raw_data;
...
static void fw_update(void *device_data)
{
	...
	case UMS:
		...
		if(fsize != info->cap_info.chip_fw_size) {
...
#endif
static ssize_t set_lowIntensity(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	u8 ret;
	...
	{
		printk("[TSP]it's wrong value !!\n");
		return -1;
	}
	if(value - 0x30)
	{
		printk("[TSP] changed LOW mode\n");
		ret = ts_write_reg(misc_touch_dev->client, 0x3C,0x0507);
		if (ret < 0) {
			dev_err(dev, "%s: err: wt reg (TSP mode1)\n",	__func__);
			return -1;
		}
		ret = ts_write_reg(misc_touch_dev->client, 0x3F,0x0104);
		if (ret < 0) {
			dev_err(dev, "%s: err: wt reg (TSP mode2)\n",	__func__);
			return -1;
		}
	}
	else
	{
		printk("[TSP] changed HIGH mode\n");
		ret = ts_write_reg(misc_touch_dev->client, 0x3C,0x0608);
		if (ret < 0) {
			dev_err(dev, "%s: err: wt reg (TSP mode3)\n",	__func__);
			return -1;
		}
		ret = ts_write_reg(misc_touch_dev->client, 0x3F,0x0103);
		if (ret < 0) {
			dev_err(dev, "%s: err: wt reg (TSP mode4)\n",	__func__);
			return -1;
		}
	}	
	return 1;
...
static ssize_t firmware_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	phone_version = (u32)((u32)(newHWID&0xff)<<16)|((newRegVersion&0xff)<<8)|(newVersion&0xff);

	printk("[TSP] phone ver firmware=ZI%06X\n", phone_version);
	return sprintf(buf, "ZI%06X\n", phone_version);
}
...
static ssize_t firmware_sw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	newVersion = misc_touch_dev->cap_info.chip_firmware_version;
	newRegVersion = misc_touch_dev->cap_info.chip_reg_data_version;
	if (ts_read_data(misc_touch_dev->client,	0x14,(u8 *)&newHWID, 2) < 0)
		printk("[TSP]read data fail! (%s)\n",__func__);
	sw_version = (u32)((u32)(newHWID&0xff)<<16)|((newVersion&0xff)<<8)|(newRegVersion&0xff);

	printk("[TSP] phone ver firmware=ZI%06X\n", sw_version);
	return sprintf(buf, "ZI%06X\n", sw_version);
}

static ssize_t tsp_threshold(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;
	u16 threshold;

	ret = ts_read_data(misc_touch_dev->client, 0x0020, (u8*)&threshold, 2);
...
static ssize_t touchkey_threshold(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;
	u16 threshold;
...
static u8 IsfwUpdate;
static ssize_t tsp_firm_update(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	printk("[TSP] %s\n",__func__);
	IsfwUpdate = 1;
	disable_irq(misc_touch_dev->irq);
...
static ssize_t tsp_firm_update_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk("[TSP] %s\n",__func__);
	return sprintf(buf, "%d\n", IsfwUpdate);
}
...
static ssize_t touchkey_menu(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;
	u16 touchkey_menu;

	ret = ts_read_data(misc_touch_dev->client, 0x00A8, (u8*)&touchkey_menu, 2);

	if (ret<0)
		return sprintf(buf, "%s\n",  "fail to read touchkey_menu.");
	else
		return sprintf(buf, "%d\n",  touchkey_menu);
}

static ssize_t touchkey_back(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;
	u16 touchkey_back;

	ret = ts_read_data(misc_touch_dev->client, 0x00A9, (u8*)&touchkey_back, 2);

	if (ret<0)
		return sprintf(buf, "%s\n",  "fail to read touchkey_back.");
	else
		return sprintf(buf, "%d\n",  touchkey_back);
}
...
static DEVICE_ATTR(touchkey_firm_version_panel , S_IRUGO | S_IWUSR | S_IWGRP , firmware_show, NULL);
static DEVICE_ATTR(touchkey_firm_version_phone , S_IRUGO | S_IWUSR | S_IWGRP , firmware_sw_show, NULL);
static DEVICE_ATTR(tsp_firm_version_panel ,S_IRUGO | S_IWUSR | S_IWGRP , firmware_show, NULL);
static DEVICE_ATTR(tsp_firm_version_phone ,S_IRUGO | S_IWUSR | S_IWGRP , firmware_sw_show, NULL);
static DEVICE_ATTR(tsp_threshold ,S_IRUGO | S_IWUSR | S_IWGRP , tsp_threshold , NULL);
static DEVICE_ATTR(touchkey_threshold ,S_IRUGO | S_IWUSR | S_IWGRP , touchkey_threshold , NULL);
static DEVICE_ATTR(tsp_firm_update ,S_IRUGO | S_IWUSR | S_IWGRP ,  NULL, tsp_firm_update);
static DEVICE_ATTR(tsp_firm_update_status ,S_IRUGO | S_IWUSR | S_IWGRP , tsp_firm_update_status , NULL);
static DEVICE_ATTR(get_touch_raw_data, S_IRUGO | S_IWUSR | S_IWGRP , zinitix_get_test_raw_data, zinitix_set_testmode);
static DEVICE_ATTR(tkey_reference, S_IRUGO | S_IWUSR | S_IWGRP , zinitix_tkey_reference, NULL);
static DEVICE_ATTR(touch_cal_ndata, S_IRUGO | S_IWUSR | S_IWGRP , zinitix_touch_cal_ndata, NULL);
static DEVICE_ATTR(delta, S_IRUGO | S_IWUSR  | S_IWGRP  , zinitix_delta, NULL);
static DEVICE_ATTR(cal_ncount, S_IRUGO | S_IWUSR | S_IWGRP , zinitix_cal_ncount, NULL);
...
static DEVICE_ATTR(count_node, S_IRUGO | S_IWUSR | S_IWGRP , zinitix_count_node, NULL);
static DEVICE_ATTR(chip_info, S_IRUGO | S_IWUSR | S_IWGRP , zinitix_chip_info, NULL);
static DEVICE_ATTR(touchkey_menu, S_IRUGO | S_IWUSR | S_IWGRP, touchkey_menu, NULL);
static DEVICE_ATTR(touchkey_back, S_IRUGO | S_IWUSR | S_IWGRP, touchkey_back, NULL);
...
static DEVICE_ATTR(touch_ndata, S_IRUGO | S_IWUSR | S_IWGRP , zinitix_touch_ndata_show, NULL);
static DEVICE_ATTR(tkey_data, S_IRUGO | S_IWUSR | S_IWGRP , zinitix_tkey_data, NULL);
static DEVICE_ATTR(set_lowIntensity, S_IRUGO | S_IWUSR | S_IWGRP , NULL, set_lowIntensity);
#endif
		...
		const struct i2c_device_id *i2c_id)
{
	...
	{
		printk("TSP isn't connected!!\n");
		return -1;
	}

	gpio_request(TSP_EN, "tsp_en");
	ts_power_control(POWER_ON);
	msleep(CHIP_ON_DELAY);
	...
	if (gpio_to_irq(touch_dev->int_gpio_num) < 0)
	...
	ret = request_threaded_irq(touch_dev->irq, ts_int_handler, zinitix_touch_work,
		IRQF_TRIGGER_LOW | IRQF_ONESHOT , ZINITIX_DRIVER_NAME, touch_dev);
#else
...
//	misc_touch_dev = touch_dev;
	...
	sec_tsp_dev = device_create(sec_class, NULL, 0, touch_dev, "tsp");

	if (IS_ERR(sec_tsp_dev))
		dev_err(&client->dev, "Failed to create device for the sysfs\n");


	ret = sysfs_create_group(&sec_tsp_dev->kobj, &sec_touch_attributes_group);
	if (ret)
	...
	if (device_create_file(sec_touchscreen_dev,
			    &dev_attr_set_lowIntensity) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_set_lowIntensity.attr.name);	
	...
	if (device_create_file(sec_touchkey_dev, &dev_attr_touchkey_firm_version_panel) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_touchkey_firm_version_panel.attr.name);
	if (device_create_file(sec_touchkey_dev, &dev_attr_touchkey_firm_version_phone) < 0)
		pr_err("Failed to create device file(%s)!\n",
	...
	if (device_create_file(sec_touchkey_dev, &dev_attr_tkey_reference) < 0)
		pr_err("Failed to create device file(%s)!\n",
