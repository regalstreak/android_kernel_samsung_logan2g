...
static irqreturn_t sci_keypad_isr(int irq, void *dev_id)
{
	...
	if ((int_status & KPD_PRESS_INT0)) {
		...
		keyreadstatus=1;
		printk("[SCI_KEY] code=%d, key state=%d\n", key, 1);
...
static __iomem void *t_base_gptimer = (__iomem void *)SPRD_GPTIMER_BASE;
#define	TIMER_LOAD(id)	(t_base_gptimer + 0x20 * (id) + 0x0000)
#define	TIMER_VALUE(id)	(t_base_gptimer + 0x20 * (id) + 0x0004)
#define	TIMER_CTL(id)	(t_base_gptimer + 0x20 * (id) + 0x0008)
#define	TIMER_INT(id)	(t_base_gptimer + 0x20 * (id) + 0x000C)
...
printk("**** INTC0_EN:0x%x   ****\n", __raw_readl( INTCV0_IRQ_EN ) );
printk("**** INTC1_EN:0x%x   ****\n", __raw_readl( INTCV1_IRQ_EN ) );

printk("**** INTCV0_IRQ_EN:0x%x ****\n",__raw_readl(INTCV0_IRQ_EN) );
if( __raw_readl(INTCV0_IRQ_EN) & IRQ_TIMER1_INT )
	printk("**** INTC0: TIMER1 IRQ is enabled ****\n");
else
	printk("**** INTC0: TIMER1 IRQ is disabled ****\n");

printk("**** INTCV0_IRQ_EN:0x%x ****\n",__raw_readl( SC8825_VA_GIC_DIS + GIC_DIST_ENABLE_SET + 0x4 ) );
if( __raw_readl( SC8825_VA_GIC_DIS + GIC_DIST_ENABLE_SET + 0x4 ) & IRQ_TIMER1_INT )
...
static int __devexit sci_keypad_remove(struct platform_device *pdev)
{
	...
	value &= ~(1 << 0);
	keypad_writel(KPD_CTRL, value);
	sci_glb_clr(REG_GLB_GEN0, GEN0_KPD_EN | GEN0_KPD_RTC_EN );
	return 0;
}
