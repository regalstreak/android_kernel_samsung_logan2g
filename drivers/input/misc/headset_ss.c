...
#define HEADSET_BUTTON_GPIO_DEBOUNCE_SW 80
#endif


#define HEADSET_DETECT_ADC_VALUE 3500
...
static void headset_mic_bias_control(int mode)
{
	...
	gpio_value=headset_gpio_get_value(HEADSET_BAIS_CTL_GPIO);

	//pr_info("[headset] before gpio_value=0x%08x \n",gpio_value);

	if(mode){
		if(gpio_value == 0x00){
			gpio_direction_output(HEADSET_BAIS_CTL_GPIO,1);
		...
		if(gpio_value == 0x01){
			gpio_direction_output(HEADSET_BAIS_CTL_GPIO,0);
...
#define HEADSET_GPIO_DEBOUNCE_SW_SAMPLE_PERIOD	20
...
static enum hrtimer_restart report_headset_button_status(int active, struct _headset_gpio *hgp)
{
	...
	unsigned int pre_ret_estate = ret_estate;
	...
	if (active < 0) {
		...
		code = KEY_RESERVED;
		ret_estate = KEY_RESERVED;
		pr_info("KEY_RESERVED eport_headset_button_status-Active [%d] [%d] \n",active, pre_ret_estate);	
...
static int report_headset_detect_status_callback(int active, struct _headset_gpio *hgp)
{
	...
	if (active) {
		...
		temp = sci_adc_get_value(ADC_CHANNEL_1, true);
		adc_value = ((2800 * (temp)) / 0x3FC);
		pr_info("[headset] ********* pole detect adc value : %d ******* \n", adc_value);
		...
		if (hgp->parent->headphone) {
			...
			msleep(300); //for prevent false button interrupt.
...
static enum hrtimer_restart headset_gpio_timer_func(struct hrtimer *timer)
{
	...
	struct _headset_gpio *hgp = container_of(timer, struct _headset_gpio, timer);
	int active = hgp->active_low ^ headset_gpio_get_value(hgp->gpio); /* hgp->active */
	int green_ch = (!active && &hgp->parent->detect == hgp) || (!active && &hgp->parent->button == hgp && ret_estate == PUSH);
...
static irqreturn_t headset_gpio_irq_handler(int irq, void *dev)
{
	...
	pr_info("%s %s\n", __func__, hgp->desc);
	wake_lock_timeout(&hgp->parent->hs_wakelock, HZ * hgp->wakelock_timeout);
	hrtimer_cancel(&hgp->timer);
	hgp->active = hgp->active_low ^ headset_gpio_get_value(hgp->gpio);
	if(&hgp->parent->detect == hgp)
	{
...
static int __init headset_init(void)
{
	...
	wake_lock_init(&ht->hs_wakelock, WAKE_LOCK_SUSPEND, "hs_wakelock");

	headset_gpio_init(ht->detect.gpio, ht->detect.desc);
	headset_gpio_init(ht->button.gpio, ht->button.desc);

	ret = gpio_request(HEADSET_BAIS_CTL_GPIO, "hs_mic_control");
	if (ret)
	{
	...
	ht->button.parent = ht;
	ht->button.wakelock_timeout = 3; // 3sec
	ht->button.irq = headset_gpio2irq(ht->button.gpio);
	ret = request_irq(ht->button.irq, headset_gpio_irq_handler,
						(IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_NO_SUSPEND) , ht->button.desc, &ht->button);
	if (ret) {
	...
	ht->detect.parent = ht;
	ht->detect.wakelock_timeout = 5; // 5 sec
	...
	ret = request_irq(ht->detect.irq, headset_gpio_irq_handler,
					(ht->detect.irq_type_active | IRQF_NO_SUSPEND), ht->detect.desc, &ht->detect);
	if (ret) {
		pr_err("request_irq gpio %d's irq failed!\n", ht->detect.gpio);
...
_gpio_request:
	...
	headset_gpio_free(HEADSET_BAIS_CTL_GPIO);
...
_switch_dev_register:
	...
	wake_lock_destroy(&ht->hs_wakelock);
	return ret;
}
