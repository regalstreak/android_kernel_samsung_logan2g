...
#ifdef SPA_BACK_CHARGING_FEATURE
#include <linux/ss6000_charger.h> 
#include <linux/rtc.h>
#include <mach/pm_debug.h>
#define SPA_TOTAL_CHARGING_TIME_LIMIT       (60*60*5)      // total charging time is  5 hour.
#define SPA_BACK_CHARGING_TIME_LIMIT         (60*40)      // back charging time is 40 minutes.
#define SPA_RESTART_CHARGING_TIME_LIMIT         (60*90)      // back charging time is 90 minutes.
u32 totalcharging_time =0;
u32 backcharging_time = 0;
u32 restartcharging_time =0;

#define SPA_STANDBY_CHARGING_THRESHOLD_HIGH    135              // 135mA
#define SPA_STANDBY_CHARGING_THRESHOLD_LOW     6                 // 6mA
int  eChargingState = SPA_STATE_NORMAL_CHARGING;
int  nCurrentChecking1= 0;
int  nCurrentChecking2= 0;
int  nCurrentChecking3= 0;
#endif
...
#if defined (CONFIG_ARCH_SC8825) || defined (CONFIG_ARCH_SC8810)
...
#define PRECHARGE_STOP_CURRENT_MAX	170
#define PRECHARGE_STOP_CURRENT_MIN	10
...
#ifndef SPA_BACK_CHARGING_FEATURE
	printk("%s voltage [%d]\n", __func__, data);
#endif
...
{
	printk("read_soc, %s, in Nevise S6818 Back charging, return\n", __func__);
	return;
}
#else
static void spa_bat_charge_full(struct spa_driver_data *info)
...
static void spa_monitor_voltage_work(struct work_struct *work)
{
	...
	static unsigned char vBatValueIndex = 0;//To avoid the soc not corrent in booting sequence.Use this value used to get soc more than 16 times, then report it.
	int soc=0, vbat=0, ret;
	static int a_VbatValues[SPA_BAT_VOLTAGE_SAMPLE]={0,};
#ifdef SPA_BACK_CHARGING_FEATURE
	struct timespec ts;
	struct rtc_time tm;
	u32 temp1 = get_sys_cnt();
	u32 temp2 = temp1 / 1000 - totalcharging_time;
	u32 temp3 = temp1 / 1000 - restartcharging_time;
	getnstimeofday(&ts);
	rtc_time_to_tm(ts.tv_sec, &tm);
	pr_info("read_soc, begin, total=%dh%02dm%02ds, restart=%dh%02dm%02ds, SysCnt=%d.%d"
		"(%d-%02d-%02d-%02d:%02d:%02d)\n",
		temp2/60/60, temp2/60%60, temp2%60,
		temp3/60/60, temp3/60%60, temp3%60,		
		temp1/1000, temp1%1000,		
...
#ifdef SPA_BACK_CHARGING_FEATURE
	printk("read_soc=%d, battery.soc=%d, average_voltage=%d, "
		"health=%d, status=%d, adc=%d, temperature=%d, end_of_charge=%d\n",
		soc, info->battery.soc, info->battery.average_voltage,
		info->battery.health, info->battery.status, info->battery.current_temp_adc, info->battery.current_temperature/1000, info->battery.end_of_charge);
	...
	if(soc <= 0)
	{
		...
		{
			unsigned int factory_exception_batt_VF;
			int factory_exception_jig_connected = spa_chg_get_jig_connected(info);
	...
	if(vBatValueIndex == SPA_BAT_VOLTAGE_SAMPLE)
	{
	        extern enum power_supply_type current_charger_type;

		if (current_charger_type == POWER_SUPPLY_TYPE_BATTERY)      // discharging
		{
			if (soc > info->battery.soc )
			{
				printk("%s,read_soc, discharging,  soc =%d, info->battery.soc=%d\n", __func__, soc, info->battery.soc);
				soc = info->battery.soc;					
			}
		}
		else if ((current_charger_type == POWER_SUPPLY_TYPE_MAINS) ||(current_charger_type == POWER_SUPPLY_TYPE_USB))    // charging with TA/USB
		{
			if (soc < info->battery.soc )
			{
				printk("%s,read_soc, charging,  soc =%d, info->battery.soc=%d\n", __func__, soc, info->battery.soc);
				soc = info->battery.soc;					
			}
		}
	}
	...
	{
		printk("%s battery full but soc is not 100 soc [%d]\n", __func__, soc);
		soc = 100;
	}
			...
			else
			{
				if((batt_VF < info->battery.VF_low) || (batt_VF > info->battery.VF_high))
				{
					...
					else
					{
						...
						{
							msleep(1000);
							spa_bat_read_voltage(info, &vbat);  // before read_VF(), need read_Voltage() first
							ret = spa_bat_read_VF(info, &batt_VF); 
							printk("%s, battery detected,i=%d,ret=%d, VF : %d, VF_low : %d, VF_high : %d\n",
								 __func__, i, ret, batt_VF, info->battery.VF_low, info->battery.VF_high);
							if ((ret >= 0) && (batt_VF >= info->battery.VF_low) && (batt_VF <= info->battery.VF_high))  // VF exist
							{
								iVFExist = 1;
							}
						}

						// if VF exist, restart charge.
						if (iVFExist == 0)
						{
							spa_bat_VF_charge_stop(info);	
							printk("%s when VF NOT exist, stop charge\n", __func__);
						}
						else
						{
							if (soc < 100)
							...
							printk("%s when soc=%d, VF exist, restart charge\n", __func__, soc);
						}
					}
#endif
				}
	...
	if(vBatValueIndex < SPA_BAT_VOLTAGE_SAMPLE)
	{
		...
		if (lpm_charging_mode)
			schedule_delayed_work(&info->monitor_VOLT_work, SPA_VOLTAGE_MONITOR_START); 
		else
...
// check recharge condition 
	...
	{
		unsigned char end_of_charge = spa_bat_clear_end_of_charge(info, SPA_END_OF_CHARGE_BY_FULL);

		if(end_of_charge == SPA_END_OF_CHARGE_NONE)
		{
#ifdef SPA_BACK_CHARGING_FEATURE
			if (eChargingState == SPA_STATE_STANDBY)
			{
				restartcharging_time = get_sys_cnt() /1000;
				eChargingState = SPA_STATE_RESTART_CHARGING;
				spa_chg_start_charge(info, SPA_CHARGE_RESTART_TIMER);
				printk("read_soc, 222,  in standby state, when voltage < %dmv,  enter restart charging, restartcharging_time=%d\n", SPA_CHARGING_RESTART_VOLTAGE, restartcharging_time); 
			}
#else
...
#ifdef SPA_BACK_CHARGING_FEATURE
	extern enum power_supply_type current_charger_type;
	printk("read_soc, end, charger=%d, State=%d, Current1=%d, Current2=%d, Current3=%d, voltage=%d\n", current_charger_type, eChargingState, nCurrentChecking1, nCurrentChecking2,nCurrentChecking3, info->battery.current_voltage);
	if((current_charger_type ==POWER_SUPPLY_TYPE_MAINS) ||(current_charger_type ==POWER_SUPPLY_TYPE_USB))
	{
		// 1. in normal charging state, when soc =100, enter back charging
		if ((info->battery.soc ==100)
 		    && (eChargingState == SPA_STATE_NORMAL_CHARGING))
		{
			eChargingState = SPA_STATE_BACK_CHARGING;     
			spa_bat_set_status(info, POWER_SUPPLY_STATUS_FULL);
			power_supply_changed(&spa_psy_bat);
			
			backcharging_time = get_sys_cnt() / 1000;
			printk("read_soc, 000, in normal charging state, when soc ==100,  enter back charging,  SysCnt=%d\n", backcharging_time); 
		}

		// 2. in back charging, when timer reached 40 minutes, stop charging and enter standby
		if ((info->battery.soc ==100)
			&&(eChargingState == SPA_STATE_BACK_CHARGING))
		{
			u32 temp = get_sys_cnt() /1000;
				
			if (temp -backcharging_time > SPA_BACK_CHARGING_TIME_LIMIT)
			{
				eChargingState = SPA_STATE_STANDBY;       // it is important to enter standby charging state and stop charge.
				spa_bat_set_end_of_charge(info, SPA_END_OF_CHARGE_BY_FULL);
				spa_chg_stop_charge(info, SPA_END_OF_CHARGE_BY_FULL);				
				printk("read_soc, 111, in back charging state, beginCnt=%d, EndCnt=%d,  stop charging.\n", backcharging_time, temp); 
			}
		}

		// 3. in restart charging, when soc ==100 &&  6mA<current < 135mA, stop charging.
		if ((info->battery.soc ==100)
		    && (eChargingState == SPA_STATE_RESTART_CHARGING)
		    && (nCurrentChecking1< SPA_STANDBY_CHARGING_THRESHOLD_HIGH)        // 135mA
		    && (nCurrentChecking1>= SPA_STANDBY_CHARGING_THRESHOLD_LOW)	// 6mA
		    && (nCurrentChecking2< SPA_STANDBY_CHARGING_THRESHOLD_HIGH)
		    && (nCurrentChecking2>= SPA_STANDBY_CHARGING_THRESHOLD_LOW)		    
		    && (nCurrentChecking3< SPA_STANDBY_CHARGING_THRESHOLD_HIGH)
		    && (nCurrentChecking3>= SPA_STANDBY_CHARGING_THRESHOLD_LOW) )
		{
			eChargingState = SPA_STATE_STANDBY;       // it is important to enter standby charging state and stop charge.
			spa_bat_set_end_of_charge(info, SPA_END_OF_CHARGE_BY_FULL);
			spa_chg_stop_charge(info, SPA_END_OF_CHARGE_BY_FULL);
			printk("read_soc, 333, in restart charging state, when charging current < %dmA,  stop charging.\n", SPA_STANDBY_CHARGING_THRESHOLD_HIGH); 
		}

		// 4. if total charging time > 5 hour, it need handle >4.15V and <=4.15V conditon
		if ((eChargingState == SPA_STATE_NORMAL_CHARGING)
			||(eChargingState == SPA_STATE_BACK_CHARGING))
		{
			u32 temp = get_sys_cnt() /1000;
			if (temp - totalcharging_time > SPA_TOTAL_CHARGING_TIME_LIMIT)
			{
				if (info->battery.current_voltage > SPA_CHARGING_RESTART_VOLTAGE)   // > 4.15V
				{
					eChargingState = SPA_STATE_STANDBY;       // it is important to enter standby charging state and stop charge.
					spa_bat_set_end_of_charge(info, SPA_END_OF_CHARGE_BY_FULL);
					spa_chg_stop_charge(info, SPA_END_OF_CHARGE_BY_FULL);	
					spa_bat_set_status(info, POWER_SUPPLY_STATUS_FULL);
					power_supply_changed(&spa_psy_bat);
					printk("read_soc, 444, if total time > 5 hour, voltage > 4.15V  stop charging.\n"); 
				}
				else        // <= 4.15V
				{
					// stop charging
					...
					eChargingState = SPA_STATE_RESTART_CHARGING;
					spa_chg_start_charge(info, SPA_CHARGE_RESTART_TIMER);
					printk("read_soc, 444,  if total time > 5 hour, voltage <= 4.15V ,  enter restart charging, restartcharging_time=%d\n", restartcharging_time); 
				}
			}
		}

		// 5. in restart charging, when timer > 90 minutes, stop charging.
		if ((info->battery.soc ==100)
		    && (eChargingState == SPA_STATE_RESTART_CHARGING))
		{
			u32 temp = get_sys_cnt() /1000;
				
			if (temp -restartcharging_time > SPA_RESTART_CHARGING_TIME_LIMIT)
			{
				eChargingState = SPA_STATE_STANDBY;       // it is important to enter standby charging state and stop charge.
				spa_bat_set_end_of_charge(info, SPA_END_OF_CHARGE_BY_FULL);
				spa_chg_stop_charge(info, SPA_END_OF_CHARGE_BY_FULL);				
				printk("read_soc, 555, in restart charging state, if > 90 minutes, beginCnt=%d, EndCnt=%d,  stop charging.\n", restartcharging_time, temp); 
			}
		}

		// 6. if battery is FULL, but soc !=100, set to 100
		if ((spa_bat_get_status(info) == POWER_SUPPLY_STATUS_FULL)
			&& (info->battery.soc !=100))
		{
			mutex_lock(&info->lock);
			info->battery.soc = 100;
			mutex_unlock(&info->lock);	
			power_supply_changed(&spa_psy_bat);
			printk("read_soc, 666, when Full state, set soc to 100\n"); 
		}
	}
#endif	
}
...
#ifdef SPA_BACK_CHARGING_FEATURE
			if ((event == SPA_DEVICE_EVENT_TA_ATTACHED)
			//	||(event == SPA_DEVICE_EVENT_TA_DETACHED)        // 0309 Lai Xiaobin Modify
				||(event == SPA_DEVICE_EVENT_USB_ATTACHED)
			//	||(event == SPA_DEVICE_EVENT_USB_DETACHED)
				||(event == SPA_DEVICE_EVENT_JIG_ATTACHED) )
			//	||(event == SPA_DEVICE_EVENT_JIG_DETACHED))
			{
				eChargingState= SPA_STATE_NORMAL_CHARGING;    
				totalcharging_time = get_sys_cnt() / 1000;
				restartcharging_time = totalcharging_time;
				printk("read_soc, event=%d, set to 0,  totalcharging_time=%d\n",event, totalcharging_time);
			}
#endif
			switch(event)
			{
...
int spa_chg_register_enable_charge(void (*enable_charge)(enum power_supply_type))
{
	...
	if(spa_chg_get_current_charger(ginfo) == SPA_CHARGER_TYPE_TA)
		...
		enable_charge(POWER_SUPPLY_TYPE_USB_DCP);	// for usb power off charging case
#else
...
static void spa_bat_recharge_start_timer_work(struct work_struct *work)
{
	...
	if(spa_bat_check_end_of_charge(info, SPA_END_OF_CHARGE_BY_TIMER)==0)
	{
		unsigned char end_of_charge = spa_bat_clear_end_of_charge(info, SPA_END_OF_CHARGE_BY_TIMER);
		if(end_of_charge == SPA_END_OF_CHARGE_NONE)
		{
...
static ssize_t fuelgauge_test_syssleep(struct device *dev,
                                   ...
                                   const char *buf, size_t count)
{
	printk("fuelgauge_test_syssleep enter, buf=%s\n", buf);

#if defined(CONFIG_SPA_FUEL_GAUGE_TEST)
	struct spa_driver_data *info = ginfo;
	if (!strncmp(buf, "pgmsleep", 8))
	{
...
#ifdef CONFIG_MACH_NEVISTD     // 20120327 BST xiaobin.lai add for SMD test sometimes sleep current 50mA problem.
		extern struct wake_lock alarm_wake_lock; 
		wake_unlock(&alarm_wake_lock);

		extern struct wake_lock rtc_wake_lock; 
		wake_unlock(&rtc_wake_lock);

		extern struct wake_lock messages_wakelock;
		wake_unlock(&messages_wakelock);

		wake_unlock(&info->battery.sleep_monitor_wakeup);
#endif		
	}
	}
#endif	
                                              ...
                                              const char *buf, size_t count)
{
	...
	if(info->use_fuelgauge)
	{
		...
		mutex_lock(&info->lock);
		info->battery.current_voltage = voltage;
		mutex_unlock(&info->lock);
#endif
	}
...
static DEVICE_ATTR(batt_reset_soc, S_IRUGO | S_IWUGO, NULL, batt_reset_soc_store_attrs);
...
#define SPA_CHECK_BATTERY_INTERVAL 358
#define SPA_CHECK_CHARGER_INTERVAL 1

int spa_check_battery(void)
{
	struct spa_driver_data *info = ginfo;
	int ret_val;
	...
	if(info)
	{	
		static int nBatteryCheckCount = 0;
		static int nChargerCheckCount = 0;
		printk("%s soc %d, nBatteryCheckCount %d, nChargerCheckCount %d\n",__func__, info->battery.soc, nBatteryCheckCount, nChargerCheckCount);
		if (info->battery.soc < 5)
		{
			printk("%s soc %d\n",__func__, info->battery.soc);
			ret_val = 1;
		}
		else if((spa_chg_get_current_charger(info) == SPA_CHARGER_TYPE_TA) && (nChargerCheckCount++ >= SPA_CHECK_CHARGER_INTERVAL))
		{
			nChargerCheckCount = 0;
			ret_val = 1;
		}	
		else if (nBatteryCheckCount++ > SPA_CHECK_BATTERY_INTERVAL)
		{
			nBatteryCheckCount = 0;
			ret_val = 1;
		}
		else
			ret_val = 0;
...
static __devinit int spa_probe(struct platform_device *pdev)
{
	...
	if (ret)
        {
                printk(KERN_ALERT "%s spa_fuelgauge_create_attrs failed\n", __func__);
		goto err_sysfs_create_group;
        }
