...
static void spa_batt_work(struct work_struct *work)
{
	...
	while(1)
	{
		...
		// Waiting for Fuelgauge
		if((int)(spa_power_iter->batt_info.capacity) < 0 && spa_power_iter->init_progress > SPA_INIT_PROGRESS_DONE)
		{
			pr_spa_dbg(LEVEL1, "%s : Waiting for Fuelgauge\n", __func__);
			msleep(100);
		}
		else
			break;
		...
		spa_power_iter->charging_status.phase == POWER_SUPPLY_STATUS_FULL))
	{
		...
		union power_supply_propval value;
		ps = power_supply_get_by_name(spa_power_iter->charger_info.charger_name);
		ps->get_property(ps, POWER_SUPPLY_PROP_CURRENT_NOW, &value);
	}
#endif	
#endif
...
int spa_event_handler(int evt, void *data)
{
	...
	if(spa_power_iter->lp_charging == 1)
		schedule_delayed_work(&spa_power_iter->battery_work, msecs_to_jiffies(100));
	else
