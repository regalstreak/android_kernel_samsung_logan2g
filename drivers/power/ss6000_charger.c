...
enum power_supply_type current_charger_type = 0;
#else
...
static void ss6000_disable_charge(unsigned char type)
{
	printk("%s\n", __func__);
...
printk("%s charger type %d\n", __func__, type);	
	...
	switch(type)
	{
		...
		case POWER_SUPPLY_TYPE_USB:
			...
			break;
		case POWER_SUPPLY_TYPE_USB_DCP:		// for usb power off charging case
			gpio_direction_output(en_set, 1);
			msleep(10);
			ss6000_set_charge_current_USB500();
			break;
...
{
	ss6000_disable_charge(POWER_SUPPLY_TYPE_BATTERY);
}
...
static void ss6000_chgsb_irq_pre_work(struct work_struct *work)
{
	struct ss6000_platform_data *pdata = container_of(work, struct ss6000_platform_data, chgsb_irq_pre_work);
	
	printk("ss6000_chgsb_irq_pre_work\n");
...
static irqreturn_t ss6000_chgsb_irq_handler(int irq, void *dev_id)
{
	...
	schedule_delayed_work(&pdata->chgsb_irq_work, SS6000_CHGSB_IRQ_DELAY);
#else
	schedule_work(&pdata->chgsb_irq_pre_work);
#endif
	return IRQ_HANDLED;
}
...
static void ss6000_pgb_irq_pre_work(struct work_struct *work)
{
	struct ss6000_platform_data *pdata = container_of(work, struct ss6000_platform_data, pgb_irq_pre_work);
	
	printk("ss6000_pgb_irq_pre_work\n");
...
static irqreturn_t ss6000_pgb_irq_handler(int irq, void *dev_id)
{
	...
	schedule_delayed_work(&pdata->pgb_irq_work, SS6000_PGB_IRQ_DELAY);
#else
	schedule_work(&pdata->pgb_irq_pre_work);
#endif
	return IRQ_HANDLED;
}
static __devinit int ss6000_probe(struct platform_device *pdev)
{
	...
	printk("%s\n",__func__);
#ifndef SPA_BACK_CHARGING_FEATURE
	INIT_WORK(&pdata->chgsb_irq_pre_work, ss6000_chgsb_irq_pre_work);
...
#endif
	INIT_WORK(&pdata->pgb_irq_pre_work, ss6000_pgb_irq_pre_work);
