...
//for config fuelgauge
extern void MainTrim(struct i2c_client * client);
...
#define STC311x_DELAY	msecs_to_jiffies(4200)
...
struct stc311x_chip {
	...
	struct timer_list		soc_protection_timer;
	int soc_protection;
	int task_ready;
#endif	
...
static void on_soc_protection_timer_expired(unsigned long x)
{
	struct stc311x_chip *chip = i2c_get_clientdata(sav_client);
	chip->soc_protection = 1;
	printk("soc_protection start\n");
}
#endif
...
int GasGauge_Task(GasGauge_DataTypeDef *GG)
{
  ...
  struct stc311x_chip *chip = i2c_get_clientdata(sav_client);
	...
	if(!timer_pending(&chip->task_timer))
		mod_timer(&chip->task_timer, jiffies + STC311x_DELAY);
...
//Check battery presence
  ...
  if (GG_Ram.reg.GG_Status == GG_INIT)
  {
    ...
    if (BattData.ConvCounter>VCOUNT) 
    {
		...
		{
			STC31xx_WriteWord(STC311x_REG_OCV,((BattData.Voltage-(BattData.Current/5))*100)/55);
		}
    }
  }
 ...
 if (BattData.AvgVoltage<APP_MIN_VOLTAGE)
	...
	//Lately fully compensation
      if(BattData.BattState > BATT_IDLE && BattData.SOC >= 990 && BattData.SOC < 995 && BattData.AvgCurrent > 120)
      {
        BattData.SOC = 990;
...
int read_soc(int *soc)
{
	...
	else
	{
		if(chip->pdata->charger_online)
		{
			if(!chip->pdata->charger_online())
			{
				if(chip->soc_protection && (pre_batt_soc < chip->batt_soc))
				{
					pr_info("%s abnormal soc SKIP pre=%d, cur=%d\n", __func__, pre_batt_soc, chip->batt_soc);
			...
			const struct i2c_device_id *id)
{
  ...
  i2c_set_clientdata(client, chip);
	...
	init_timer(&chip->soc_protection_timer);
	chip->soc_protection_timer.function = on_soc_protection_timer_expired;
	chip->soc_protection_timer.data = (unsigned long)NULL;
	chip->soc_protection = 0;
	mod_timer(&chip->soc_protection_timer, jiffies + msecs_to_jiffies(60000)); /* 60sec*/
#endif
