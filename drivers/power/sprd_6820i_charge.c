...
void sprd_vbatvolt_checkpoint(uint32_t volt_level, uint32_t adc_value)
{
	...
	if (unlikely((volt_level > CHGMNG_ULTRA_VOLTAGE || adc_value > 4090) && state == 0)) {
		...
		state = 1;
		sci_adi_write(ANA_REG_GLB_DCDC_CTRL1, DCDC_PDRSLOW(3), MASK_DCDC_PDRSLOW);
		sci_adi_write(ANA_REG_GLB_DCDCARM_CTRL1, DCDC_PDRSLOW(3), MASK_DCDC_PDRSLOW);
		sci_adi_write(ANA_REG_GLB_DCDCMEM_CTRL1, DCDC_PDRSLOW(3), MASK_DCDC_PDRSLOW);
		sci_adi_write(ANA_REG_GLB_DCDCLDO_CTRL1, DCDC_PDRSLOW(3), MASK_DCDC_PDRSLOW);
		if (pm_power_off)
			pm_power_off();
	}
	else if (state != 0 && volt_level < (PREVCHGEND + 50)) {
		state = 0;
		sci_adi_write(ANA_REG_GLB_DCDC_CTRL1, DCDC_PDRSLOW(0), MASK_DCDC_PDRSLOW);/*default*/
		sci_adi_write(ANA_REG_GLB_DCDCARM_CTRL1, DCDC_PDRSLOW(0), MASK_DCDC_PDRSLOW);
		sci_adi_write(ANA_REG_GLB_DCDCMEM_CTRL1, DCDC_PDRSLOW(0), MASK_DCDC_PDRSLOW);
		sci_adi_write(ANA_REG_GLB_DCDCLDO_CTRL1, DCDC_PDRSLOW(0), MASK_DCDC_PDRSLOW);
	}
#endif
	return;
}
