...
static int sprd_get_current (unsigned int opt)
{
	...
	if(opt == 1)
	{
		...
		if(battery_data->batt_eoc_count >= 0)
		{
			...
			if(battery_data->batt_eoc_count > 4)
			{
...
{
	int temp_value = sci_adc_get_value(ADC_CHANNEL_TEMP, false);
	if (temp_value >= 0)
...
static int plugin_callback(int usb_cable, void *data)
{
	...
	wake_lock_timeout(&(d->charger_plug_out_lock), CONFIG_PLUG_WAKELOCK_TIME_SEC * HZ);

	if(lp_boot_mode && d->charging)
		return 0;
...
static int detect_device(void)
{
	...
	if (jig_on)
	{
		mdelay(VCHG_READ_DELAY);
		adc = sci_adc_get_value(ADC_CHANNEL_VCHG, false);
		printk("detect_device ADC_CHANNEL_VCHG %d\n", adc);
