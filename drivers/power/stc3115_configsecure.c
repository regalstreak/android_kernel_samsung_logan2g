...
void ReadRAM(void)
{
	unsigned char RxBuffer[10];
	unsigned char IDCode;
	printk("STC_FUSING ReadRAM enter\n");
	I2C_ReadByte(0xE0, 0x18, &IDCode, 1);
	if (IDCode == 0x13) {
		printk("STC_FUSING ReadRAM IDC code is 0x13\n");
		...
		printk("STC_FUSING ReadRAM IDC code is not 0x13\n");
...
void ExitTest(void)
{
	...
	printk("STC_FUSING ExitTest enter\n");
	I2C_ReadByte(0xE0, 0x18, &IDCode, 1);
	if (IDCode == 0x13) {
		printk("STC_FUSING ExitTest IDC code is 0x13\n");
		RxBuffer[0]=0x00;
		I2C_WriteByte(0xE0,0x46,RxBuffer,1); 
		RxBuffer[0]=0x00;
		I2C_WriteByte(0xE0,0x43,RxBuffer,1); 
		...
		printk("STC_FUSING ExitTest IDC code is not 0x13\n");
		/*exit test mode*/
		RxBuffer[0]=0x00;
		I2C_WriteByte(0xE0,0x46,RxBuffer,1); 
		RxBuffer[0]=0x00;
...
{
	unsigned char RxBuffer[10];
	/*needed only if read procedure is different between cut. Today the procedure is identical*/
	unsigned char i;
	...
	I2C_WriteByte(0xE0,0x46,RxBuffer,1);
	msleep(5);
	I2C_ReadByte(0xE0, 0x20, SectorData, 8);
	for (i=0; i < 8; i++)
		printk("readsector%d[%d]=0x%x\n",SectorNum, i, SectorData[i]);
}
...
*******************************************************************************/
int MainTrimTest(struct i2c_client *client)
{
#if 1
	unsigned char IDCode; 
	...
	return -1;
#endif  //#if defined(CONFIG_MACH_NEVISTD)
...
ReadRAM(); //Enter test mode
 ReadSector(0, Sector0);  //read sector 0
 ReadSector(1, Sector1);  //read sector 1
 ReadSector(2, Sector2);  //read sector 2
 ReadSector(3, Sector3);  //read sector 3
 ReadSector(4, Sector4);  //read sector 4
 ReadSector(5, Sector5);  //read sector 5
 ReadSector(6, Sector6);  //read sector 6
 ReadSector(7, Sector7);  //read sector 7
 ExitTest();            //exit test mode
