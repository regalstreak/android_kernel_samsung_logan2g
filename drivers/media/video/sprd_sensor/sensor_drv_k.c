...
LOCAL int _Sensor_K_SetVoltage(SENSOR_SET_VOLTAGE_T v_val)
{
	...
	if(v_val.iodd_val == SENSOR_VDD_3800MV) iodd_val = SENSOER_VDD_3800MV;
	else if(v_val.iodd_val == SENSOR_VDD_3000MV) iodd_val = SENSOER_VDD_3000MV;
	else if(v_val.iodd_val == SENSOR_VDD_2800MV) iodd_val = SENSOER_VDD_2800MV;
    else if(v_val.iodd_val == SENSOR_VDD_2500MV) iodd_val = SENSOER_VDD_2500MV;
	else if(v_val.iodd_val == SENSOR_VDD_1800MV) iodd_val = SENSOER_VDD_1800MV;
	else if(v_val.iodd_val == SENSOR_VDD_1500MV) iodd_val = SENSOER_VDD_1500MV;
	else if(v_val.iodd_val == SENSOR_VDD_1300MV) iodd_val = SENSOER_VDD_1300MV;
	else if(v_val.iodd_val == SENSOR_VDD_1200MV) iodd_val = SENSOER_VDD_1200MV;
	else iodd_val = 0;

	if(v_val.avdd_val == SENSOR_VDD_3800MV) avdd_val = SENSOER_VDD_3800MV;
	else if(v_val.avdd_val == SENSOR_VDD_3000MV) avdd_val = SENSOER_VDD_3000MV;
	else if(v_val.avdd_val == SENSOR_VDD_2800MV) avdd_val = SENSOER_VDD_2800MV;
    else if(v_val.avdd_val == SENSOR_VDD_2500MV) avdd_val = SENSOER_VDD_2500MV;
	else if(v_val.avdd_val == SENSOR_VDD_1800MV) avdd_val = SENSOER_VDD_1800MV;
	else if(v_val.avdd_val == SENSOR_VDD_1500MV) avdd_val = SENSOER_VDD_1500MV;
	else if(v_val.avdd_val == SENSOR_VDD_1300MV) avdd_val = SENSOER_VDD_1300MV;
	else if(v_val.avdd_val == SENSOR_VDD_1200MV) avdd_val = SENSOER_VDD_1200MV;
	else avdd_val = 0;	

	if(v_val.dvdd_val == SENSOR_VDD_3800MV) dvdd_val = SENSOER_VDD_3800MV;
	else if(v_val.dvdd_val == SENSOR_VDD_3000MV) dvdd_val = SENSOER_VDD_3000MV;
	else if(v_val.dvdd_val == SENSOR_VDD_2800MV) dvdd_val = SENSOER_VDD_2800MV;
    else if(v_val.dvdd_val == SENSOR_VDD_2500MV) dvdd_val = SENSOER_VDD_2500MV;
	else if(v_val.dvdd_val == SENSOR_VDD_1800MV) dvdd_val = SENSOER_VDD_1800MV;
	else if(v_val.dvdd_val == SENSOR_VDD_1500MV) dvdd_val = SENSOER_VDD_1500MV;
	else if(v_val.dvdd_val == SENSOR_VDD_1300MV) dvdd_val = SENSOER_VDD_1300MV;
	else if(v_val.dvdd_val == SENSOR_VDD_1200MV) dvdd_val = SENSOER_VDD_1200MV;	
	else dvdd_val = 0;


	/* GET Regulator CAM IO, AVDD, DVDD*/
	SENSOR_PRINT("SENSOR:Get regulator \n");
...
/* DISABLE CAM IO,AVDD,DVDD VOLTAGE*/
	SENSOR_PRINT("SENSOR: _Sensor_K_SetVoltage dvdd_val =%d,avdd_val =%d,iodd_val = %d .\n",dvdd_val,avdd_val,iodd_val);

if((dvdd_val == 0) || (avdd_val == 0) || (iodd_val == 0)){

		SENSOR_PRINT("SENSOR:All Power disable  = %d .\n");
	...
	/* SET CAM IO, CAM AVDD,DVDD SET VOLTAGE*/
	SENSOR_PRINT("SENSOR:Set regulator voltage s_camdvdd_regulator=%d,s_camavdd_regulator =%d,s_camvio_regulator = %d .\n",s_camdvdd_regulator,s_camavdd_regulator,s_camvio_regulator);

	err = regulator_set_voltage(s_camvio_regulator,iodd_val,iodd_val);
	if(err)
	{
		goto CAM_IO_ERR;
	}
	err = regulator_set_voltage(s_camavdd_regulator, avdd_val,avdd_val);
	if(err)
	{
		goto CAM_IO_ERR;
	}
		err = regulator_set_voltage(s_camdvdd_regulator,dvdd_val,dvdd_val);
	if(err)
	{
		goto CAM_IO_ERR;
	}
	...
	/* ENABLE CAM IO,AVDD,*/ 
	SENSOR_PRINT("SENSOR:Enable Regulator \n");
	...
	SENSOR_PRINT_ERR("SENSOR:Sensor Power Enable done(success) %d .\n");
	return SENSOR_K_SUCCESS;
...
LOCAL int _Sensor_K_Reset(uint32_t level, uint32_t width)
{
	...
	mdelay(1);
#ifndef CONFIG_MACH_NEVISTD
       val = __raw_readl( CTL_PIN_BASE + REG_PIN_CTRL1  );
       SENSOR_PRINT("%x\n",val);
       val = val | 1 << 29;
       SENSOR_PRINT("%x\n",val);
	val = val | 1 << 30;
	SENSOR_PRINT("%x\n",val);
  	 __raw_writel( val, CTL_PIN_BASE + REG_PIN_CTRL1 );
#endif	
	return SENSOR_K_SUCCESS;
}
...
int SF_init_burst_write(SENSOR_REG_T_PTR reg_table, uint32_t init_table_size);
...
LOCAL int _Sensor_K_WriteRegTab(SENSOR_REG_TAB_PTR pRegTab)
{
	...
	SENSOR_PRINT_ERR("sensor_k_write start:size = %d \n", cnt);
	...
	if(0 == pRegTab->burst_mode){
		...
		ret = SF_init_burst_write(sensor_reg_ptr, pRegTab->reg_count);
...
_Sensor_K_WriteRegTab_return:
	...
	printk("_Sensor_K_WriteRegTab: done, ret = %d, cnt=%d, time=%d us \n", ret, cnt,
...
LOCAL int _Sensor_K_SetI2CClock(uint32_t clock)
{
	...
	printk("_Sensor_K_SetI2CClock: set i2c clock to %d  \n", clock);
...
//#define USE_MALLOC
#define BURST_MODE_BUFFER_MAX_SIZE 256*2*sizeof(uint16_t)
uint8_t temp_buf_for_burstmode[BURST_MODE_BUFFER_MAX_SIZE];
...
int SF_init_burst_write(SENSOR_REG_T_PTR reg_table, uint32_t init_table_size)
{
	...
	uint8_t  *tmp_data_for_burst;
	uint32_t current_count = 0;

	SENSOR_REG_BITS_T addr_data = {0,};
    struct timeval time1, time2;	
	...
	printk("++++SENSOR: SF_init_burst_write E ++++\n");

	if(0 == i2c_client)
	{
		printk("SENSOR: Burstmode:error,i2c_client is NULL!.\n");
	}

#ifdef USE_MALLOC
	tmp_data_for_burst = (uint8_t*)kzalloc(256*2*sizeof(uint16_t), GFP_KERNEL);
	
	if(PNULL == tmp_data_for_burst){
		SENSOR_PRINT_ERR("Burstmode_buffer ERROR:kmalloc is fail, size = %d \n",256*2*sizeof(uint16_t) + 16);
		return -1;
	}
	else{
		SENSOR_PRINT_HIGH("Burstmode_buffer: kmalloc success, size = %d \n", 256*2*sizeof(uint16_t) + 16);
	}
#endif

I2C_RETRY:

	while(current_count < init_table_size)
	{

		addr_data.reg_addr = reg_table[current_count].reg_addr;
		addr_data.reg_value = reg_table[current_count].reg_value;

        if(burst_on)
        {
			if(addr_data.reg_addr == 0x0e && addr_data.reg_value == 0x00)
			{
				//SENSOR_PRINT_HIGH("SF_init_burst_write: start burst write address = %x, count = %d\n",i2c_client->addr,burst_count);			
				msg_w.addr = i2c_client->addr;
				msg_w.flags = 0;
#ifdef USE_MALLOC				
				msg_w.buf = tmp_data_for_burst;
#else
				msg_w.buf = temp_buf_for_burstmode;
#endif
				msg_w.len = (uint32_t)(burst_count);
				//SENSOR_PRINT_HIGH("SF_init_burst_write:burst[0] = 0x%x\n",tmp_data_for_burst[0]);
				//SENSOR_PRINT_HIGH("SF_init_burst_write:burst[1] = 0x%x\n",tmp_data_for_burst[1]);	
				//SENSOR_PRINT_HIGH("SF_init_burst_write:burst[end] = 0x%x\n",tmp_data_for_burst[burst_count-2]);					
				//SENSOR_PRINT_HIGH("SF_init_burst_write:burst[end] = 0x%x\n",tmp_data_for_burst[burst_count-1]);				

				ret = i2c_transfer(i2c_client->adapter, &msg_w, 1);

#ifdef USE_MALLOC				
				memset((void*)tmp_data_for_burst,0,256*2*sizeof(uint16_t));
#else
				memset((void*)temp_buf_for_burstmode,0,256*2*sizeof(uint16_t));
#endif
				...
				burst_on = 0; 				
					...
					if(ret <0)
					{
						SENSOR_PRINT_HIGH("SF_init_burst_write: normal write error\n");				
						break;
			}
		...
		else
		{
			...
			{
#ifdef USE_MALLOC					
					tmp_data_for_burst[0] = (uint8_t)(addr_data.reg_addr);
#else
					temp_buf_for_burstmode[0] = (uint8_t)(addr_data.reg_addr);
#endif

					burst_count = 1;
				}
#ifdef USE_MALLOC					
					tmp_data_for_burst[burst_count] = (uint8_t)(addr_data.reg_value);

#else
					temp_buf_for_burstmode[burst_count] = (uint8_t)(addr_data.reg_value);
		...
		{
			if ((0x0e == addr_data.reg_addr) && (0x01 == addr_data.reg_value))
			{
				//SENSOR_PRINT_HIGH("*****Burst mode on******\n");			
				burst_on = 1;
			}
			else if((0xff == addr_data.reg_addr) && (0xff != addr_data.reg_value))
			{
				//SENSOR_PRINT_HIGH("SF_init_burst_write: delay = 0x%x\n",addr_data.reg_value);			
				if(addr_data.reg_value >= 10)
				{
					msleep(addr_data.reg_value);
				}
				else
				{
					mdelay(addr_data.reg_value);
				}
			}
			else if((0xff == addr_data.reg_addr) && (0xff == addr_data.reg_value))
			{
				//SENSOR_PRINT_HIGH("*****finished send Init Data ******\n");
				break;
			}
			else
			{
				addr_data.reg_bits  = SENSOR_I2C_REG_8BIT;
				//if(addr_data.reg_addr == 0x3)
				//SENSOR_PRINT_HIGH("SF_init_burst_write: normal write addr = 0x%x,data = 0x%x\n",addr_data.reg_addr,addr_data.reg_value);				
				ret = _Sensor_K_WriteReg(&addr_data);
	...
	if (unlikely(ret < 0))
		{
            printk("[SENSOR]%s: set failed by burst mode.\n",__func__);
			retry++;
    ...
    do_gettimeofday(&time2);
#ifdef USE_MALLOC		
		kfree(tmp_data_for_burst);
#endif
		printk("SENSOR:Burst init time=%d.\n",((time2.tv_sec-time1.tv_sec)*1000+(time2.tv_usec-time1.tv_usec)/1000));
    	printk("SENSOR:Burst write success, success X\n");
			  ...
			  unsigned long arg)
{
	...
	//SENSOR_PRINT_ERR("sensor_k_ioctl cmd = %d \n",cmd);	

	switch (cmd) {
	case SENSOR_IO_PD:
		{
			BOOLEAN power_level;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_PD \n");				
	...
	case SENSOR_IO_SET_CAMMOT:
		{
			uint32_t vdd_val;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_SET_CAMMOT \n");					
	...
	case SENSOR_IO_SET_AVDD:
		{
			uint32_t vdd_val;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_SET_AVDD \n");					
			ret = copy_from_user(&vdd_val, (uint32_t *) arg, sizeof(uint32_t));
			if(0 == ret)
	...
	case SENSOR_IO_SET_DVDD:
		{
			uint32_t vdd_val;
		        //SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_SET_DVDD \n");					
			ret = copy_from_user(&vdd_val, (uint32_t *) arg, sizeof(uint32_t));
			if(0 == ret)
	...
	case SENSOR_IO_SET_IOVDD:
		{
			uint32_t vdd_val;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_SET_IOVDD \n");					
			ret = copy_from_user(&vdd_val, (uint32_t *) arg, sizeof(uint32_t));
			if(0 == ret)
	...
	case SENSOR_IO_SET_VOLTAGE:
		{
			SENSOR_SET_VOLTAGE_T v_val;
			SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_SET_VOLTAGE \n");					
			ret = copy_from_user(&v_val, (SENSOR_SET_VOLTAGE_T *) arg, sizeof(SENSOR_SET_VOLTAGE_T));
			if(0 == ret)
				ret = _Sensor_K_SetVoltage(v_val);
		}
		break;
	...
	case SENSOR_IO_SET_MCLK:
		{
			uint32_t mclk;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_SET_MCLK \n");					
	...
	case SENSOR_IO_RST:
		{
			uint32_t rst_val[2];
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_RST \n");					
	...
	case SENSOR_IO_I2C_INIT:
		{
			uint32_t sensor_id;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_I2C_INIT \n");				
	...
	case SENSOR_IO_I2C_DEINIT:
		{
			uint32_t sensor_id;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_I2C_DEINIT \n");					
			ret = copy_from_user(&sensor_id, (uint32_t *) arg, sizeof(uint32_t));
			if(0 == ret)
	...
	case SENSOR_IO_SET_ID:
		{
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_SET_ID \n");					
	...
	case SENSOR_IO_RST_LEVEL:
		{
			uint32_t level;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_RST_LEVEL \n");					
	...
	case SENSOR_IO_I2C_ADDR:
		{
			uint16_t i2c_addr;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_I2C_ADDR \n");					
	...
	case SENSOR_IO_I2C_READ:
		{
			...
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_I2C_READ \n");					
	...
	case SENSOR_IO_I2C_WRITE:
		{
			SENSOR_REG_BITS_T reg;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_I2C_WRITE \n");					
			ret = copy_from_user(&reg, (SENSOR_REG_BITS_T *) arg, sizeof(SENSOR_REG_BITS_T));
	...
	case SENSOR_IO_SET_FLASH:
		{
			uint32_t flash_mode;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_SET_FLASH \n");					
	...
	case SENSOR_IO_I2C_WRITE_REGS:
		{
			...
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_I2C_WRITE_REGS \n");				
	...
	case SENSOR_IO_SET_I2CCLOCK:
		{
			uint32_t clock;
			//SENSOR_PRINT_ERR("sensor_k_ioctl SENSOR_IO_SET_I2CCLOCK \n");					
...
//set ccirpd0 and ccirrst output 0 for IORA test
static void camera_gpio_init(void)
{
	int err;
	printk("%s: set main camea sensor pwn and rst out 0   --system dev \n", __func__);
	err = gpio_request(GPIO_MAIN_SENSOR_PWN, "main camera");	//ccirpd0
	if (err) {
		printk("main camera failed requesting err=%d\n", err);
	}
	gpio_direction_output(GPIO_MAIN_SENSOR_PWN, 0);
	gpio_free(GPIO_MAIN_SENSOR_PWN);

	err = gpio_request(GPIO_SENSOR_RESET, "ccirrst");		//ccirrst
	if (err) {
...
{ 
	...
	return  sprintf(buf, SENSOR_NAME);
}
