...
#if defined(CONFIG_MACH_NEVISTD)
	//Zhouym Add For DTP Mode
	if(INVALID_VALUE != g_dcam_info.dtp_param)
                Sensor_Ioctl(SENSOR_IOCTL_SET_DTP, g_dcam_info.dtp_param);
#endif
}
...
static int init_sensor_parameters(void *priv)
{
...
#if defined(CONFIG_MACH_NEVISTD)
	if(SENSOR_MODE_MAX == i){
		g_dcam_info.preview_m = SENSOR_MODE_PREVIEW_ONE;
		g_dcam_info.snapshot_m = SENSOR_MODE_PREVIEW_ONE;
		g_dcam_info.input_size.w = fh->width;
		g_dcam_info.input_size.h = fh->width;
	}

	printk("init_sensor_parameters -- cs --: preview_m=%d, snapshot_m=%d \n",
	...
	#if defined(CONFIG_MACH_NEVISTD)
       /*for preview *///Songww 20130320 add for 720*480 record
	if((1 != dev->streamparm.parm.capture.capturemode) && (g_dcam_info.snapshot_m != SENSOR_MODE_RECORDING_TWO) ){
	...
	Sensor_SetModeEX(init_param.input_size.w, init_param.input_size.h, dev->streamparm.parm.capture.capturemode);
			  ...
			  struct v4l2_cropcap *cc)
{
	...
	cc->bounds.left = 0;
	cc->bounds.top = 0;
	cc->bounds.width = 2048;
	cc->bounds.height = 1536;
			 ...
			 struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
		case V4L2_CID_GET_EXIF_ISO_SPEED:
		{
			...
			retval = Sensor_Ioctl(SENSOR_IOCTL_GET_EXIF_ISO, 0);
			ctrl->value = retval;
			break;
		}
		...
		{
			printk( "[DHL]V4L2_CID_GET_EXIF_EXPOSURETIME_DENOMINAL.. \n");
			retval = Sensor_Ioctl(SENSOR_IOCTL_GET_EXIF_EXPOSURE, 0);
			ctrl->value = retval;
			break;
		}
		...
		{
			printk( "[DHL]V4L2_CID_GET_EXIF_EXPOSURETIME_DENOMINAL.. \n");
			retval = Sensor_Ioctl(SENSOR_IOCTL_GET_EXIF_FLASH, 0);
			ctrl->value = retval;
			break;
		}
	default:
...
static int vidioc_handle_ctrl(struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_GAIN:
		...
		break;
#if defined(CONFIG_MACH_NEVISTD)
	case V4L2_CID_SET_FPS:
		dcam_stop_handle(is_previewing);
		g_dcam_info.fps_param =  (uint8_t) ctrl->value;
		Sensor_Ioctl(SENSOR_IOCTL_SET_FPS, (uint32_t) ctrl->value);
		dcam_start_handle(is_previewing);
		break;
	...
	case	V4L2_CID_DTP:
		...
		if(g_dcam_info.dtp_param == (uint8_t)ctrl->value)
                {
                        DCAM_V4L2_PRINT("V4L2:don't need handle dtp!.\n");
                        break;
                }
                g_dcam_info.dtp_param = (uint8_t)ctrl->value;
                dcam_stop_handle(is_previewing);
...
#if defined(CONFIG_MACH_NEVISTD)
       /*for preview *///Songww 20130320 add for 720*480 record
	 if( (1 == init_param.mode)&&(g_dcam_info.snapshot_m!= SENSOR_MODE_RECORDING_TWO)){
#else
...
#if defined(CONFIG_MACH_NEVISTD)
	//Zhouym Add For DTP Mode
	g_dcam_info.dtp_param = INVALID_VALUE;
