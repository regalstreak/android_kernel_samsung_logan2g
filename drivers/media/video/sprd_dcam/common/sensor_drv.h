...
typedef enum {
	...
	SENSOR_MODE_RECORDING_TWO,//Songww 20130320 add for 720*480 record
...
typedef enum {
	...
	SENSOR_IOCTL_ZOOM,
	SENSOR_IOCTL_CUS_FUNC_3,//35
	SENSOR_IOCTL_FOCUS,
	SENSOR_IOCTL_ANTI_BANDING_FLICKER,
	SENSOR_IOCTL_VIDEO_MODE,
	SENSOR_IOCTL_PICK_JPEG_STREAM,
	SENSOR_IOCTL_SET_MERTERING_MODE,//40
#if defined(CONFIG_MACH_NEVISTD)
	SENSOR_IOCTL_SET_FPS,
	SENSOR_IOCTL_GET_EXIF_ISO,
	SENSOR_IOCTL_GET_EXIF_EXPOSURE,
	SENSOR_IOCTL_GET_EXIF_FLASH,
	SENSOR_IOCTL_SET_DTP,//45
#endif
	SENSOR_IOCTL_MAX
} SENSOR_IOCTL_CMD_E;
...
typedef struct sensor_ioctl_func_tab_tag {
	...
	uint32_t(*reset) (uint32_t param);//0
	...
	uint32_t(*read_reg) (uint32_t param);//5
	/*Custom function 6 */
	...
	uint32_t(*vmirror_enable) (uint32_t param);//10
	...
	uint32_t(*set_preview_mode) (uint32_t param);//15
	...
	uint32_t(*read_ae_value) (uint32_t param);//20
	...
	uint32_t(*set_frame_rate) (uint32_t param);//25
	...
	uint32_t(*set_iso) (uint32_t param);//30
	...
	/*CUSTOMER FUNCTION */
	uint32_t(*get_exif) (uint32_t param);//35
	...
	uint32_t(*set_meter_mode) (uint32_t param);//40
#if defined(CONFIG_MACH_NEVISTD)
	uint32_t(*set_fps) (uint32_t param);
	uint32_t(*get_exif_iso) (uint32_t param);
	uint32_t(*get_exif_exporsure) (uint32_t param);
	uint32_t(*get_exif_flash) (uint32_t param);
