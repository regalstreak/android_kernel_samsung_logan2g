...
#if defined(CONFIG_MACH_NEVISTD) //bst chris.shen modify for sensor I2c write 20130126
LOCAL void _Sensor_SetI2CSpeed(uint32_t set_i2c_clock);
// bst chris.shen add patch for sensor_drv.c:1467:4: error: implicit declaration of function 'sc8810_i2c_set_clk' [-Werror=implicit-function-declaration]
...
int32_t Sensor_regs_table_init(void)
{
	...
	filp = filp_open("/data/log/sensor_regs.h", O_RDONLY, 0);
	if (IS_ERR(filp)) {
...
int32_t Sensor_regs_table_write(char *name)
{
	...
	while (1) {
		...
		if (reg == NULL) {
			printk(KERN_ERR " EXCEPTION! addr : 0x%x,  value : 0x%x\n", addr, value);
			break;
		}
		       ...
		       SENSOR_AVDD_VAL_E iodd_val)
{
	...
	udelay(600);
	printk("SENSOR:add wait 600us for power timing spec before VDD2.8\n");//Songww 20130407 add wait for power timing spec VDDC and VDD2.8
	...
	if (0 != volt_value) {
		...
		udelay(600);
		printk("SENSOR:add 600us wait for power timing spec\n");//Songww 20130322 add wait for power timing spec
...
#if defined(CONFIG_MACH_NEVISTD)
extern SENSOR_INFO_T g_sr300pc20_yuv_info;
LOCAL int  _Sensor_Identify(SENSOR_ID_E sensor_id)
{
	...
	printk("SENSOR:new sensor identifing %d", sensor_id);

//	sensor_index = 0;//s_sensor_index[sensor_id];
	...
	sensor_info_ptr = &g_sr300pc20_yuv_info;
	...
	if (PNULL != sensor_info_ptr->ioctl_func_tab_ptr->identify) {
		...
		printk("SENSOR:identify  Sensor 01\n");
		if (SENSOR_SUCCESS == sensor_info_ptr-> ioctl_func_tab_ptr->identify(SENSOR_ZERO_I2C))
		{
			...
			printk("_Sensor_Identify:sensor_id :%d,img_sensor_num=%d,sensor_index=%d.\n",  sensor_id,     s_sensor_register_info_ptr->img_sensor_num,sensor_index);
		}
	}
	else
	{
...
IDENTIFY_SEARCH:
	...
	if (SCI_TRUE == s_sensor_register_info_ptr->is_register[sensor_id]) {
		...
		printk("%s[%d] : skip except sensor id 0(%d)\n",__func__,__LINE__,sensor_id) ;
#endif
	}
}
#endif
...
LOCAL void _Sensor_SetStatus(SENSOR_ID_E sensor_id)
{
	...
	SENSOR_PRINT("_Sensor_SetStatus: 1");
		...
		if(SENSOR_TRUE == sensor_register_info_ptr->is_register[sensor_id])
		{
			...
			SENSOR_PRINT_HIGH("SENSOR: Sensor_sleep of id %d",sensor_id);
	}
...
//	BOOLEAN reg_flag =  SCI_TRUE;
	...
	SENSOR_PRINT("STEP 1");
	 ...
	 if(_Sensor_Register(SENSOR_MAIN) != SENSOR_SUCCESS)
	 {
		return SENSOR_FAIL;
	 }
	...
	if(SCI_TRUE != _Sensor_Identify(SENSOR_MAIN))
	{
...
#if defined(CONFIG_MACH_NEVISTD)
int Sensor_SetModeEX(uint32_t width, uint32_t height, uint32_t capture_mode)
{
	SENSOR_EXP_INFO_T *sensor_info_ptr = NULL;
	uint32_t size_mode;
	SENSOR_MODE_E mode = -1;

	sensor_info_ptr = Sensor_GetInfo();
	if (PNULL == sensor_info_ptr) {
		printk("v4l2:Sensor_SetModeEX,get sensor info fail .\n");
		return -1;
	}

	printk("v4l2: Sensor_SetModeEX: width=%d, height=%d, capture_mode=%d \n", width, height, capture_mode);

	if ((SENSOR_IMAGE_FORMAT_RAW != sensor_info_ptr->image_format)
	    && (1 == capture_mode))
	{

		if(width ==  2048 && height == 1536) 			size_mode = (uint32_t)0;
		else if(width == 2048  && height == 1232)  	size_mode = (uint32_t)1;
		else if(width ==  1600  && height == 1200)  	size_mode = (uint32_t)2;
		else if(width == 1280    && height == 960 ) 	size_mode = (uint32_t)4;
		else if(width == 800    && height == 480 )  	size_mode = (uint32_t)8;
		else if(width == 720    && height == 480 ) 		size_mode = (uint32_t)9;
 		else if(width == 640   && height == 480) 		size_mode = (uint32_t)10;
		else if(width == 320    && height == 240) 		size_mode = (uint32_t)13;
		else if(width == 176   && height == 144)		size_mode = (uint32_t)14;
		else size_mode = (uint32_t)0;
		printk("V4L2: sensor_ioctl_before_snapshot: = %d\n",size_mode);
		Sensor_Ioctl(SENSOR_IOCTL_BEFORE_SNAPSHOT,(uint32_t)size_mode);
		//s_sensor_mode[Sensor_GetCurId()] = 2;
	}
	else
	{
		if((width == 2048  && height == 1232) || (width == 800 && height == 480 ) )
		{
			size_mode = 2;
		}
		else if(width == 720 && height == 480 )  size_mode = (uint32_t)0;
		else if(width == 320 && height == 240 )  size_mode = (uint32_t)5;
		else if(width == 176 && height == 144 )  size_mode = (uint32_t)6;
		else
		{
			size_mode= 1;
		}
		printk("V4L2: sensor_ioctl_after_snapshot: = %d\n",size_mode);
		Sensor_Ioctl(SENSOR_IOCTL_AFTER_SNAPSHOT, (uint32_t) size_mode);
		return 0;
	}
...
#ifdef CONFIG_LOAD_FILE
	result = Sensor_regs_table_init();
	if (result > 0)
	{
		SENSOR_PRINT(KERN_ERR "***** sensor_regs_table_init  FAILED. Check the Filie in MMC\n");
		return result;
	}
	result =0;
            if(SENSOR_MODE_COMMON_INIT == mode)
				Sensor_regs_table_write("reg_main_init");
			else
				Sensor_regs_table_write("reg_main_preview");

#else
#if defined(CONFIG_MACH_NEVISTD)
		SENSOR_PRINT(KERN_ERR "***** %s[%d] :  setmode by cus burst func : %d \n",__func__,__LINE__,mode);
...
#else
		SENSOR_PRINT(KERN_ERR "***** %s[%d] :  setmode : %d \n",__func__,__LINE__,mode);
		Sensor_SendRegTabToSensor
		    (&s_sensor_info_ptr->resolution_tab_info_ptr[mode]);
...
#if defined(CONFIG_MACH_NEVISTD) //bst chris.shen modify for sensor I2c write 20130126
	s_sensor_register_info_ptr->is_register[SENSOR_MAIN] = SCI_FALSE;
	s_sensor_register_info_ptr->is_register[SENSOR_SUB] = SCI_FALSE;
	s_sensor_register_info_ptr->is_register[SENSOR_ATV] = SCI_FALSE;
#endif
	return SENSOR_SUCCESS;
}
