...
#define SPRDFB_ESD_TIME_OUT_CMD	(3000)

#define SPRDFB_ESD_TIME_OUT_VIDEO	(3000)

#define GPIO_LCD_EN		172
...
int lcd_get_power(struct lcd_device *dev)
{
	return 0;
}


static struct lcd_ops sprd_lcd_ops = {
...
static void ESD_work_func(struct work_struct *work)
{
	...
	if(0 != dev->enable){
		...
		printk("sprdfb: DON't reschedule ESD workqueue since device not available!!\n");
	}
...
/* + CABC Mode Sequence */
static LCM_Init_Code cabc_ui[] = { /* 0x55 */
	{LCM_SEND(2), {0x55, 0x01}},
};

static LCM_Init_Code cabc_still[] = { /* 0x55 */
	{LCM_SEND(2), {0x55, 0x02}},
};

static LCM_Init_Code cabc_moving[] = { /* 0x55 */
...
/* + CABC off Sequence */
static LCM_Init_Code cabc_off[] = { /* 0x55 */
	{LCM_SEND(2), {0x55, 0x00}},
};
/* - CABC off Sequence */

void set_seq(struct panel_spec *self, LCM_Init_Code *lcd_seq)
{
	unsigned int tag;
	int32_t i;
	int32_t temp;

	LCM_Init_Code *seq=lcd_seq;
	...
	for(i = 0; i < self->seq_size; i++){
		tag = (seq->tag >>24);
		if(tag & LCM_TAG_SEND){
			mipi_gen_write(seq->data, (seq->tag & LCM_TAG_MASK));
			udelay(90);
		}else if(tag & LCM_TAG_SLEEP){
			msleep(seq->tag & LCM_TAG_MASK);
		}
		seq++;
	}
}
...
void ldi_cabc_on(struct panel_spec *self)
{
	
	printk("%s\n", __func__);

	self->seq_size= ARRAY_SIZE(cabc_ui);

	if(self->cabc==CABC_UI)
		set_seq(self, cabc_ui);
	else if(self->cabc==CABC_STILL)
		set_seq(self, cabc_still);	
	else if(self->cabc==CABC_MOVING)
		set_seq(self, cabc_moving);
	else
		set_seq(self, cabc_ui);	

}

void ldi_cabc_off(struct panel_spec *self)
{
	printk("%s\n", __func__);

	self->seq_size= ARRAY_SIZE(cabc_off);
	self->cabc=CABC_OFF;	
	
	set_seq(self, cabc_off);
}


void set_cabc_value(struct panel_spec *self, u8 force)
{
	printk("%s, self->cabc = %d\n",__func__, self->cabc);
...
ssize_t cabc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sprdfb_device *sprdfb_dev = dev_get_drvdata(dev);
...
#else
	printk("%s, cabc=%d\n",__func__, sprdfb_dev->panel->cabc);
 	return sprintf(buf, "%d\n", sprdfb_dev->panel->cabc);

#endif
}
 
ssize_t cabc_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct sprdfb_device *sprdfb_dev =  dev_get_drvdata(dev);
	...
	self->cabc=value;
 	...
 	set_cabc_value(self, 0);
 #else
	sprdfb_dev->panel->cabc=value;
  	set_cabc_value(sprdfb_dev->panel, 0);
 #endif
...
static int sprdfb_probe(struct platform_device *pdev)
{
	...
	struct class *sprdfb_class;

	sprdfb_class = class_create(THIS_MODULE, "sprdfb_class");
	...
	ld = lcd_device_register("panel", &pdev->dev, NULL, &sprd_lcd_ops);

	if (IS_ERR(ld))
	{
		printk("lcd_device_register error!!");
	}
	...
	dev_t = device_create( sprdfb_class, &pdev->dev, 0, "%s", "status");

 	dev_set_drvdata(dev_t, dev);


	if (device_create_file(dev_t, &dev_attr_cabc) < 0)
