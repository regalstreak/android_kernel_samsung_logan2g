...
extern int32_t sprdfb_dsi_gen_write(uint8_t *param, uint16_t param_length);
...
int mdnie_send_sequence(struct mdnie_info *mdnie, LCM_Init_Code *seq)
{
 	int ret = 0;
   ...
   ret = sprdfb_dsi_gen_write(seq->data,(seq->tag & LCM_TAG_MASK));
 ...
 static int ldi_cabc_on(void)
{
   ...
   ret = sprdfb_dsi_gen_write(cabc_on02->data,(cabc_on02->tag & LCM_TAG_MASK));
   ret = sprdfb_dsi_gen_write(cabc_on03->data,(cabc_on03->tag & LCM_TAG_MASK));
   ret = sprdfb_dsi_gen_write(cabc_ui->data,(cabc_ui->tag & LCM_TAG_MASK));
...
static int ldi_cabc_off(void)
{
   ...
   ret = sprdfb_dsi_gen_write(cabc_off->tag,(cabc_off->tag & LCM_TAG_MASK));
...
void mdnie_late_resume(struct early_suspend *h)
{
   ...
   //   set_cabc_value(mdnie, 0);
      ...
      sprdfb_dsi_gen_write(&conversion,1);
#endif
