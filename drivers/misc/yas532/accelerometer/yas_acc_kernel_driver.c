...
#elif defined CONFIG_YAS_ACC_DRIVER_BMA222E
...
static int yas_acc_set_delay(struct yas_acc_driver *driver, int delay)
{
	...
	printk(KERN_WARNING "acc] %s : %d ms, (Actual)%d ms\n", __func__,delay,actual_delay(delay));
...
static int bma222_fast_calibration(int is_cal_erase)
{
	...
	{
#ifdef CONFIG_YAS_ACC_DRIVER_LIS3DH
		layout[2] = YAS_LIS3DH_RESOLUTION*yas_lis3dh_position_map[CONFIG_INPUT_YAS_ACCELEROMETER_POSITION][2][2];	
#else
		...
		else 
		{
...
#ifdef CONFIG_YAS_ACC_DRIVER_LIS3DH
		 	if (yas_lis3dh_read_reg(YAS_LIS3DH_ACC_REG, buf, 6) != 0) 
#else
		 	...
		 	else 
		 	{
        		...
        		#if defined(CONFIG_SENSORS_K3DH) || defined(CONFIG_YAS_ACC_DRIVER_LIS3DH)
		        	cal_data.x = raw[0]*16 - layout[0];
		        	cal_data.y = raw[1]*16 - layout[1];
		        	cal_data.z = raw[2]*16 - layout[2];
			#else					
			 ...
			 const struct i2c_device_id *id)
{
	...
	calRestored = 0;	
	printk(KERN_INFO "ACC] %s started\n", __func__);
	...
	printk(KERN_INFO "ACC] %s finished.\n", __func__);
#ifdef CONFIG_MULTI_ACC_GARDA
	isAccFound = 1;
#endif	
	return 0;
