...
#define VBC_PCM_REVERT_LEFT_RIGHT_CHANNEL_DA 1
#endif
			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	int used_chan_count;
//wendonghan
#if VBC_PCM_REVERT_LEFT_RIGHT_CHANNEL_DA
	int revert_channel_da = 0;
#endif
	sprd_pcm_dbg("Entering %s\n", __func__);
...
//wendonghan
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		revert_channel_da = 1;
		pr_info("vbc fix VBIIS_LRCK must be 0 hardware bug, we swap DA0 & DA1 DMA buffer!\n");
	}
#endif
	...
	rtd->da0_offset = rtd->da1_offset = totsize;
...
//wendonghan
#if VBC_PCM_REVERT_LEFT_RIGHT_CHANNEL_DA
	if (revert_channel_da) 
	{
		dma_buff_phys[1] = runtime->dma_addr;
		rtd->dma_addr_offset = (totsize / used_chan_count);

		if (sprd_pcm_is_interleaved(runtime))
		rtd->dma_addr_offset = 2;
		dma_buff_phys[0] = runtime->dma_addr + rtd->dma_addr_offset;
...
//wendonghan
	data_base = runtime->dma_addr;
#if VBC_PCM_REVERT_LEFT_RIGHT_CHANNEL_DA
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {

		data_base += rtd->dma_addr_offset;
	}
#endif

	if (rtd->uid_cid_map[0] >= 0) {

		offset= sprd_pcm_dma_get_addr(rtd->uid_cid_map[0],
						    substream) -
		   data_base;
	...
	if (rtd->uid_cid_map[1] >= 0) {
		/* Assign DA0 offset */
		if (unlikely(offset - rtd->da0_offset < 0))
			rtd->da0_wrap = 1; /* DA0 buffer_size wrap jump */
		rtd->da0_offset = offset;

		/* Assign DA1 offset */
		offset = sprd_pcm_dma_get_addr(rtd->uid_cid_map[1],
						    substream) - runtime->dma_addr - rtd->dma_addr_offset;
		if (unlikely(offset - rtd->da1_offset < 0))
			rtd->da1_wrap = 1; /* DA1 buffer_size wrap jump */
		rtd->da1_offset = offset;

		if (unlikely(rtd->da0_wrap ^ rtd->da1_wrap)) {
			/* One of DA0 or DA1 wrap around happen */
			offset = max(rtd->da0_offset, rtd->da1_offset);
		} else {
			if (unlikely(rtd->da0_wrap)) {
				rtd->da0_wrap = rtd->da1_wrap = 0;
				snd_printd("Both of DA0 & DA1 buffer size are wraped around\n");
			}
			offset = min(rtd->da0_offset, rtd->da1_offset);
		}
		offset <<= shift;
	}
