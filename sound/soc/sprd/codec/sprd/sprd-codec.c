...
#ifdef CONFIG_SPRD_AUDIO_DEEP_DEBUG
...
static inline void sprd_codec_pa_ovp_v_sel(int v_sel)
{
	int mask;
	int val;
	sprd_codec_dbg("Entering %s set %d\n", __func__, v_sel);
	...
	arch_audio_codec_write_mask(PMUR6, val, mask);
}
...
int sprd_inter_speaker_pa(int on)
{
	...
	if (on) {
		sprd_codec_pa_ovp_v_sel(PA_OVP_465);
...
static int sprd_codec_analog_open(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, AUD_SDM_CTL0, 0x300);
#else
...
#else //for CONFIG_SND_SPRD_SOC_BT_I2S
...
#endif //for CONFIG_SND_SPRD_SOC_BT_I2S
...
static int sprd_codec_is_linein_function(int id)
{
	switch (id) {
	case ID_FUN(SPRD_CODEC_AIL, SPRD_CODEC_LEFT):
	case ID_FUN(SPRD_CODEC_AIL, SPRD_CODEC_RIGHT):
	case ID_FUN(SPRD_CODEC_AIR, SPRD_CODEC_LEFT):
	case ID_FUN(SPRD_CODEC_AIR, SPRD_CODEC_RIGHT):
		return 1;
	default: break;
	}
	return 0;
...
static int sprd_codec_linein_special_action(struct snd_soc_codec *codec, int on)
{
	if (on) {
		snd_soc_dapm_enable_pin(&codec->card->dapm, "V_DAC");
	} else {
		snd_soc_dapm_disable_pin(&codec->card->dapm, "V_DAC");
	}
	return 1;
}
#endif
...
#ifdef CONFIG_MACH_LOGAN
	if (sprd_codec_is_linein_function(id)) {
		sprd_codec_linein_special_action(codec, mixer->on);
	}
#endif
...
#ifdef CONFIG_MACH_LOGAN
	/* this a virsual DAC for speacial case */
	SND_SOC_DAPM_INPUT("V_DAC"),
#endif
};
...
#endif
#ifdef CONFIG_MACH_LOGAN
	{"Digital DACL Switch", NULL, "V_DAC"},
	{"Digital DACR Switch", NULL, "V_DAC"},
#endif
};
...
#ifdef CONFIG_MACH_LOGAN
	snd_soc_dapm_disable_pin(&codec->card->dapm, "V_DAC");
	snd_soc_dapm_ignore_suspend(&codec->card->dapm, "V_DAC");
#endif
	sprd_codec_dbg("return %i\n", ret);
	sprd_codec_dbg("Leaving %s\n", __func__);
	return ret;
